...
| DS-33 | Tax exemption cert file | Tax Exemption Cert | `file_mm55t6kn` (`COLS.taxExemptCertFile`) | File | Customer (via portal) | `[PARTIAL — mapped 2026-07-27]` Column exists live and is now mapped in `lib/monday.js`; the portal upload control on SCR-9 is not yet built — see PLAN-8 |
| DS-34 | Tax exemption status | Tax Exempt | `color_mm55tjn2` (`COLS.taxExemptStatus`) | Status | Staff (review) | `[PARTIAL — mapped 2026-07-27]` Pre-existing column, labels are **No / Yes / No Response / Incomplete** (not the No Certificate/Pending Review/Approved/Expired set PLAN-8 originally envisioned) — portal will display this label read-only; staff continues to manage it in Monday |
...
| PLAN-8 | **Tax Exemption Certificate upload (SCR-9)** | `[UNBLOCKED 2026-07-27 — upload endpoint still to build]` | Add a "Tax Exemption Certificate" module to the Invoice & Payment tab. Customer uploads via a file control → writes DS-33 (file) and posts tagged update `[PORTAL: Tax Exemption Certificate Uploaded]` + EM-14 to team. Staff reviews and sets DS-34 (status) in Monday. Portal shows the current status back to the customer plus a plain-language disclosure: without an approved certificate on file, sales tax is applied to the invoice. DS-33/DS-34 columns already existed live and are now mapped in `lib/monday.js` (2026-07-27) — that was the only blocker noted here previously. **Remaining:** the actual upload endpoint. Unlike EP-11 (staff pastes an already-hosted URL) or `attachUgcFile` (URLs already hosted by Jotform), this needs the customer to upload a file directly from their device — real multipart upload handling (`bodyParser: false`, forward to Monday's file-upload API), not the URL-string pattern used elsewhere in this codebase. Worth its own focused build rather than reusing the URL-based helpers as-is. |
