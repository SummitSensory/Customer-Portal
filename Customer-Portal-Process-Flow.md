...
| PLAN-10 | **Staff setup-progress dashboard + assisted-entry mode** | `[PROGRESS DASHBOARD LIVE 2026-07-27]` | Bryan's concern: customers who struggle with the portal call in for help, and staff had no quick way to see what's incomplete. Two-part design: (1) **Progress dashboard** — read-only 5-dot readout (DS-39) on the Admin Orders and Customers tabs showing Contact/Billing/Delivery/Colors/Documents status per order (`ProgressDots` component). Shipped 2026-07-27, no new auth surface. (2) **Full assisted-entry mode** — `[PLANNED, deferred]` a "View/Assist as Customer" action that would drop staff into the actual customer portal UI scoped to an order, with a visible "Staff Assisted Session" banner and an audit note logged to Monday, so staff can literally fill in fields for the customer over the phone. Deferred as a separate build (new session/auth logic) until the dashboard alone is evaluated. |
