...
| OPEN-3 | Message Status doesn't flip to Replied when staff reply directly in Monday.com | `[PARTIAL — code fixed 2026-07-28, automation still needs manual registration]` | Bryan reported replying to a customer inside Monday.com left Message Status (DS-40) stuck on "Needs Reply." Two separate causes found: (1) the **Admin Portal reply path** had a real code bug — `pages/api/monday/messages.js` only ever set Message Status on the *customer* POST branch; a staff reply from the Admin Portal Messages tab never cleared the flag. **Fixed 2026-07-28** — a staff branch now sets it to Replied. (2) The **raw Monday Updates reply path** (replying directly on the item in Monday.com, not through the Admin Portal) depends entirely on `update-webhook.js` (EP-14), which only fires if a Monday automation "When an update is created → send a webhook → `https://portal.summitsensory.com/api/monday/update-webhook`" is registered on this board. Vercel production logs show **zero requests** to that endpoint in the trailing 24 hours, confirming the automation does not currently exist (three unrelated "Webhooks" app automations already on the board, created Nov 2025–Feb 2026, are a different integration — most likely the AfterShip/Zapier accessory-tracking push from PLAN-2/EP-22 — and are not this trigger). Attempted to register it via the monday.com MCP connector's automation-builder: it correctly resolved the trigger ("When update created") but the outgoing-webhook action requires selecting a pre-authorized destination through Monday's own Webhooks-app UI — it won't accept a freeform URL via the API, the same limitation already documented for PLAN-7. **Remaining manual step:** on the Manufacturing Process board (`6533700776`) → Automations → search "webhook" → the recipe "When an update is created, send a webhook" → paste `https://portal.summitsensory.com/api/monday/update-webhook` → save/authorize. No secret/signature is required for this endpoint. Until that's done, replying to a customer directly in Monday's Updates feed will not flip the flag or email the customer (EM-11) — use the Admin Portal Messages tab for now, which works today. **Third bug found and fixed 2026-07-28 (full QA pass), same file:** `update-webhook.js` had its own hardcoded staff-domain check (`STAFF_DOMAIN` defaulting to `summitsensorygym.com` only) instead of the shared `isStaffEmail()` helper in `lib/auth.js` (default `summitsensory.com,summitsensorygym.com`). Bryan's own address (`bryan@summitsensory.com`) is on the domain the old fallback excluded — if `STAFF_EMAIL_DOMAIN` isn't set in Vercel, his own replies once this automation is registered would have been silently misread as non-staff. Fixed by importing `isStaffEmail()` instead; verified via esbuild in both repos. |
