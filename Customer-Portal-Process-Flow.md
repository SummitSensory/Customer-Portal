...
| PLAN-7 | **Auto-send portal invite on "Send Invite" status** | `[CODE COMPLETE 2026-07-27 — Monday automation registration remains]` | When a Manufacturing item's invite-status column (`color_mm5427cr`, "Customer Portal Invite") is set to **"Send Invite"**, `pages/api/monday/invite-webhook.js` fires the invite (EM-02 + `[PORTAL: Invitation Sent]`) and flips the column to "Invite Sent" to prevent duplicates. Fixed 2026-07-27: the webhook called `setStatusLabel()`/referenced `COLS.inviteStatus`, neither of which existed in `lib/monday.js` — both now added (`setStatusLabel(itemId, colKey, label)` mirrors `markSectionComplete`'s pattern; `COLS.inviteStatus` confirmed live against board 6533700776 via `color_mm5427cr`, labels "Send Invite"/"Do Not Send"/"TBD"). **Remaining:** register the Monday automation itself — When [Customer Portal Invite] changes to "Send Invite" → Send a webhook → `https://portal.summitsensory.com/api/monday/invite-webhook?secret=MONDAY_INVITE_SECRET`. Until that automation exists, changing the column does nothing; the code fix alone doesn't create the trigger. |
