...
| DS-40 | Message queue status | Message Status | `color_mm5pny4b` (`COLS.messageStatus`) | Status | Staff (view) | `[LIVE 2026-07-27]` New column, created via monday.com MCP. Flips to **Needs Reply** when a customer sends a portal message (EP-12) and back to **Replied** when staff reply (EP-14, either via Monday Updates natively or the Admin Messages tab). Admin Messages tab (SCR — Messages) sorts Needs-Reply threads to the top with a red dot; Messages nav tab shows a count badge. Filter a Monday view on this column for a native support queue — no new board needed. |
