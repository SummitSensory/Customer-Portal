...
export const SHIPMENT_LABELS = { frame: 'Sensory Gym Frame', mats: 'Therapy Mats & Padding' };

/** Builds an AfterShip tracking title that disambiguates Frame vs. Mats on
 * orders with both, instead of sending the identical order name for both. */
export function buildTrackingTitle(orderName, shipmentKey) {
...
async function ensureTrackingId(slug, trackingNumber, { title, orderId, customerName, contacts } = {}) {
  ...
  // customers/title were never applied. Update the existing tracking so they
  // appear — this backfills shipments onboarded before a delivery contact was
  // available (or before contacts were sent to AfterShip at all).
  const alreadyExisted = res.status !== 201 && id;
  if (alreadyExisted && (customers.length || title)) {
    const update = {};
