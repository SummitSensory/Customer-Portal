...
// PORTAL-050: each 429 retry above previously got its own FRESH
// AFTERSHIP_FETCH_TIMEOUT_MS (15s) window, with no cap tracked across the
// whole retry sequence for a single logical call — under sustained
// rate-limiting, up to AFTERSHIP_MAX_RETRIES (3) retries, each with its own
// full 15s fetch timeout plus a retry-after wait in between, could push one
// logical call (one fetchWithTimeout entry point, e.g. ensureTrackingId's
// create POST) past 60+ seconds — likely past Vercel's own function timeout,
// which would kill the invocation before this file's own graceful fallback
// (trackShipment/onboardShipment's outer catch, returning null/pending()
// instead of throwing) ever got a chance to run. `deadline` is an absolute
// Date.now()-based timestamp (real application code, not a workflow script,
// so using Date.now() directly is fine and expected here) computed ONCE on
// the outermost call of a retry sequence and threaded through every
// recursive retry via the 4th argument, rather than recomputed fresh each
// time. Each attempt's own fetch timeout is capped to whatever time remains
// until that shared deadline (never more than AFTERSHIP_FETCH_TIMEOUT_MS),
// and once the deadline itself has already passed — or a retry-after wait
// would clearly overshoot it — the function gives up immediately instead of
// granting yet another attempt a full fresh window.
async function fetchWithTimeout(url, options = {}, attempt = 0, deadline = null) {
  const callDeadline = deadline ?? (Date.now() + AFTERSHIP_FETCH_TIMEOUT_MS);
  const remainingMs = callDeadline - Date.now();
  if (remainingMs <= 0) {
    throw new Error(`AfterShip request exceeded its cumulative retry deadline (${AFTERSHIP_FETCH_TIMEOUT_MS}ms budget) after ${attempt} attempt(s).`);
  }

  const controller = new AbortController();
  const timeoutMs = Math.min(AFTERSHIP_FETCH_TIMEOUT_MS, remainingMs);
  ...
  if (res.status === 429 && attempt < AFTERSHIP_MAX_RETRIES) {
    ...
    // Don't sleep out a retry-after wait that would land past the deadline
    // anyway — fail fast now instead of waiting, then immediately hitting
    // the deadline check above on the recursive call with nothing to show
    // for the wait.
    if (Date.now() + waitMs >= callDeadline) {
      throw new Error(`AfterShip request rate-limited (429) and the next retry-after wait would exceed the cumulative retry deadline; giving up after ${attempt + 1} attempt(s).`);
    }

    await new Promise(r => setTimeout(r, waitMs));
    return fetchWithTimeout(url, options, attempt + 1, callDeadline);
  }
