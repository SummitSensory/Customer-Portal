...
async function ensureTrackingId(slug, trackingNumber, { title, orderId, customerName } = {}) {
  ...
  if (customerName) body.customer_name = String(customerName);
  ...
  if (alreadyExisted && (customerName || title)) {
    const update = {};
    if (customerName) update.customer_name = String(customerName);
    if (title) update.title = String(title);
    try {
      await fetch(`${AFTERSHIP_BASE}/trackings/${encodeURIComponent(id)}`, {
        method: 'PUT',
        headers: authHeaders(),
        body: JSON.stringify(update),
      });
    } catch (e) {
      console.error('AfterShip update (customer_name) failed:', e.message);
    }
  }
