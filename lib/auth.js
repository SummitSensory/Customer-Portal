 ...
 * `attempts` (PORTAL-008) tracks failed verification attempts against this
 * specific code so verify-code.js can lock a code out after too many wrong
 * guesses, instead of allowing unlimited retries against a 6-digit space.
 */
export async function signCodeToken(email, code, attempts = 0) {
  return new SignJWT({ email, code, attempts })
