...
export const BOARDS = {
  gb: {
    ...
    dealsLink: 'connect_boards_Mjj6Hm3f', // GB "Deals"
  },
  r: {
    ...
    dealsLink: 'link_to_deals5__1', // R "Deals"
  },
  acc: {
...
const GB_SLIDE_LABELS = new Set(['Green', 'Blue']);
// Marks a row the portal filled, so GB's Prismatic formula prices it per
// unique color (what the customer was shown) — see "Prismatic Charge Count".
const GB_UPCHARGE_RULE = 'text_mm7gyr52';
...
const R_TEXT = {
  ...
  // "Soar Column Wraps Color" (added by Bryan 2026-09-24).
  [`${COLOR_INPUT.SOAR_MAT}.column_wraps`]: env('MONDAY_R_COL_SOAR_COLUMN_WRAPS', 'text_mm7g4k9f'),
...
export function planBoardWrites(order, requiredInputs, selections) {
  ...
  if (slide) {
    // A label the column doesn't have would fail the whole row, paint included.
    const name = colorName(slide);
    if (GB_SLIDE_LABELS.has(name)) gb[GB_SLIDE_COLOR] = { label: name };
    else skipped.push(`GB: slide color "${name}" is not a label on SLIDE - Color`);
  }
  if (Object.keys(gb).length) {
    gb[GB_REQUEST] = { label: RECEIVED };
    gb[GB_UPCHARGE_RULE] = 'Per unique color';
    plan.gb = gb;
  }
...
const MFG_DEALS_LINK = 'link_to_deals__1'; // Manufacturing Process → Deal Tracking

async function mfgLinks(orderId) {
  const cols = [...Object.values(BOARDS).map((b) => b.mfgLink), MFG_DEALS_LINK];
...
// Every row on the board whose back-link to Manufacturing includes this order,
// with all of that row's back-linked order ids and its creation time.
async function rowsBackLinkedTo(board, orderId) {
  const found = [];
  let cursor = null;
  let first = true;
  do {
    const data = first
      ? await mondayQuery(`
          query($b: [ID!], $col: [String!]) {
            boards(ids: $b) { items_page(limit: 500) { cursor items { id created_at column_values(ids: $col) { ... on BoardRelationValue { linked_item_ids } } } } }
          }
        `, { b: [board.id], col: [board.backLink] })
      : await mondayQuery(`
          query($c: String!, $col: [String!]) {
            next_items_page(cursor: $c, limit: 500) { cursor items { id created_at column_values(ids: $col) { ... on BoardRelationValue { linked_item_ids } } } }
          }
    ...
    for (const item of page?.items || []) {
      ...
      if (ids.includes(String(orderId))) found.push({ id: String(item.id), createdAt: item.created_at, backLinks: ids });
    }
...
// Other Manufacturing orders whose forward link points at this row.
async function otherMfgOrdersLinkingTo(board, rowId, orderId) {
  const mfgBoardId = process.env.MONDAY_BOARD_ID;
  if (!mfgBoardId) return [];
  const others = [];
  let cursor = null;
  let first = true;
  do {
    const data = first
      ? await mondayQuery(`
        ...
        `, { b: [mfgBoardId], col: [board.mfgLink] })
      : await mondayQuery(`
          query($c: String!, $col: [String!]) {
            next_items_page(cursor: $c, limit: 500) { cursor items { id column_values(ids: $col) { ... on BoardRelationValue { linked_item_ids } } } }
          }
        `, { c: cursor, col: [board.mfgLink] });
    const page = first ? data.boards?.[0]?.items_page : data.next_items_page;
    first = false;
    for (const item of page?.items || []) {
      if (String(item.id) === String(orderId)) continue;
      const ids = (item.column_values?.[0]?.linked_item_ids || []).map(String);
      if (ids.includes(String(rowId))) others.push(String(item.id));
    }
    cursor = page?.cursor || null;
  } while (cursor);
  return others;
...
/**
 * Choose the row to write for this order on one board, or null to create one.
 *
 * A row is only reused if it belongs to THIS order alone. Live data has rows
 * shared between two orders (a "(copy)" order, a second order for the same
 * customer) — writing one order's colors there would overwrite the other's,
 * so a shared row is left alone and a new row is created instead. When several
 * rows belong to this order (Jotform resubmissions), the newest is used.
 */
async function chooseRow(board, orderId, linked, notes) {
  const onMfg = (linked[board.mfgLink] || []).filter((it) => String(it.board?.id) === String(board.id));
  if (onMfg.length) {
    const rowId = String(onMfg[onMfg.length - 1].id);
    const back = await backLinkIds(board, rowId);
    const otherBack = back.filter((id) => id !== String(orderId));
    const otherFwd = await otherMfgOrdersLinkingTo(board, rowId, orderId);
    if (!otherBack.length && !otherFwd.length) return { rowId, back };
    notes.push(`${board.label}: the row linked to this order (${rowId}) is shared with other order(s) ${[...new Set([...otherBack, ...otherFwd])].join(', ')}, so it was left alone and a new row was created`);
    return null;
  }
  const rows = (await rowsBackLinkedTo(board, orderId)).filter((r) => r.backLinks.every((id) => id === String(orderId)));
  if (!rows.length) return null;
  rows.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  if (rows.length > 1) notes.push(`${board.label}: ${rows.length} rows belong to this order (${rows.map((r) => r.id).join(', ')}); the newest (${rows[0].id}) was updated`);
  return { rowId: rows[0].id, back: rows[0].backLinks };
}

/**
 * Write confirmed selections onto GB / R / Accessories. Returns a per-board
 * summary; per-board failures are collected in `errors` so one board failing
 * never blocks the others, and `notes` explains any judgement calls (shared
 * rows, several candidate rows).
 */
export async function syncConfirmedColorsToBoards(order, requiredInputs, selections) {
  ...
  const result = { boards: {}, errors: [], skipped, notes: [] };
  if (!Object.keys(plan).length) return result;

  const mfgBoardId = process.env.MONDAY_BOARD_ID;
  const linked = await mfgLinks(order.id);
  const dealIds = (linked[MFG_DEALS_LINK] || []).map((it) => Number(it.id));

  for (const [key, values] of Object.entries(plan)) {
    const board = BOARDS[key];
    try {
      const chosen = await chooseRow(board, order.id, linked, result.notes);
      let itemId;
      let created = false;
      if (chosen) {
        itemId = chosen.rowId;
        const write = chosen.back.includes(String(order.id))
          ? values
          : { ...values, [board.backLink]: { item_ids: [Number(order.id)] } };
        await setColumns(board.id, itemId, write);
      } else {
        // The name carries a marker so board automations that link rows to
        // Manufacturing / Deals BY NAME can't attach it to a different,
        // same-named order; the links are set explicitly instead.
        const newValues = { ...values, [board.backLink]: { item_ids: [Number(order.id)] } };
        if (board.dealsLink && dealIds.length) newValues[board.dealsLink] = { item_ids: dealIds };
        itemId = await createRow(board.id, `${order.name || order.id} — portal`, newValues);
        created = true;
      }
