 ...
 * photo; Prismatic's 378 colors were pulled from prismaticpowders.com with
 * no photos available (their CDN blocks hotlinking), so those are shown as
 * verified swatches only. Originally 428 — direct requirement (2026-09-03):
 * 50 colors with no derivable finish (see derivePrismaticFinish below) were
 * removed from the catalog outright, not just left unclassified. Those 50
 * came from Prismatic's old, retired River-line catalog page; a live
 * spot-check on 3 of them found zero matches on Prismatic's current site,
 * suggesting (not proven for all 50) they may no longer be real, orderable
 * products at all.
 *
...
import ballPitBallData from './data/ballPitBallColors.json';
...
// 378 SKUs literally start "RAL-"). Direct requirement from Bryan
...
// Vein, Wrinkle) occur in any of the 378 names. Every RAL-prefixed SKU's
// name also happens to contain "River" (170 of 170), so RAL is checked
// first and wins over the River match.
//
// 50 colors originally had neither RAL nor "River" in the name and so had
// NO real signal to classify by — a live spot-check (searching Prismatic's
// current site for a few of them, e.g. "Black Rock" / PRB-1019) found no
// matching product at all under that name. That dataset was pulled from
// Prismatic's old River-line catalog page (see lib/colorCatalogSync.js's
// header comment — that page 404s today), and those 50 SKU prefixes
// (PRB/PRS/HRB/URB/PLB/ERB) don't match the live site's current prefix
// scheme (PSS/PMS/PTB/etc. observed live). Rather than show a color with a
// fabricated finish, Bryan's direct instruction (2026-09-03) was to remove
// those 50 from the catalog entirely — see lib/data/prismaticColors.json,
// now 378 entries (was 428). derivePrismaticFinish's `return null` below is
// kept as defensive code, not dead code: it still governs what happens if a
// future catalog sync (lib/colorCatalogSync.js) ever reintroduces an entry
// with no derivable finish — never fabricate a customer-facing
// classification with zero underlying evidence (see this module's header
// on provenance).
...
// Ball Pit Balls' own real color set — originally pixel-sampled from Bryan's
// marketing chart "Therapy Ball Pit - Ball Color Selection.png" (2026-09-09);
// superseded 2026-09-09 by Bryan's own "Ball Pit Ball Colors - Hex Code.xlsx"
// (lib/data/Ball-Pit-Balls-Swatches/), an exact hex reference for all 13
// named colors — more authoritative than a pixel-sampled approximation, so
// every value here now matches that spreadsheet exactly. This is a
// DIFFERENT, distinct palette from VINYL_COLORS above — the two are not
// interchangeable even though both ultimately trace back to real supplier
// swatches, because these are the specific colors the actual balls come in,
// not the mat/pad vinyl material. 12 of the spreadsheet's 13 named colors
// are here; "Clear" — now a known, real value (#FFFFFF00, an 8-digit hex
// with the alpha channel at 0 — fully transparent) — is still deliberately
// NOT included, because every consumer of `.hex` here renders a solid CSS
// background, which would misrepresent transparency as opaque white. Needs
// a UI decision (e.g. a distinct transparent/checkered treatment) before it
// can be added — the blocker is the rendering, not missing data anymore.
//
// Landing this catalog does NOT unblock Ball Pit Balls in
// lib/colorRequirements.js on its own — that product still needs its own
// input paradigm (a per-color 0-2000 quantity allocator, not a single-color
// picker) plus new Monday storage/validation/pricing, all still unscoped.
export const BALL_PIT_BALL_COLORS = ballPitBallData.map((d) => ({
  ...d,
  brand: 'ball_pit_balls',
}));

// Slide plastic colors — real names given directly by Bryan (2026-09-21):
// Blue and Green. Gray (+$500) was removed as an option by Bryan on
// 2026-09-23 (the GB board's "SLIDE - Color" column only has Green/Blue);
// no saved selection used it. The `upcharge` field and its pricing branch
// are kept so a future paid color needs no code change.
// A different catalog from VINYL_COLORS above — the slide is plastic, not
// vinyl-wrapped material (see ALLOWED_BRANDS in lib/colorRequirements.js).
// `upcharge` is a NEW field, unlike every catalog above it: those all price
// exclusively off `brand` (Prismatic = $500/$300 uniqueness rule, vinyl =
// $0) via computeLineItemPricing's brand check, but Gray's charge is a
// flat per-color amount unrelated to brand-level rules, so it lives on the
// color entry itself. Hex values are representative approximations for the
// swatch UI, NOT pixel-sampled from a real photo like every other catalog
// in this file — flagged plainly via provenance: 'bryan-provided' (not
// 'photo-verified'/'swatch-verified') so this stays visibly distinct if a
// real photo/hex ever replaces it.
export const SLIDE_COLORS = [
  { name: 'Blue', hex: '#2E5FA3', upcharge: 0 },
  { name: 'Green', hex: '#2F6B3C', upcharge: 0 },
].map((d) => ({ ...d, brand: 'plastic', provenance: 'bryan-provided' }));

// Foundation System mat colors — real options given directly by Bryan
// (2026-09-22), shown when the "Foundation System-Mat Color" gate
// (color_mm7c5nq2) is Included: Black/Gray, Red/Blue, Green/Gray. These are
// reversible two-tone interlocking foam tiles (one color per face), not the
// single-color vinyl in VINYL_COLORS — a separate catalog with its own
// brand, so a vinyl name can never validate against a Foundation part (see
// ALLOWED_BRANDS in lib/colorRequirements.js) or vice versa. No upcharge was
// given for any of the three, so none is priced (computeLineItemPricing
// only charges prismatic/plastic). Photos are Bryan's own supplier images
// (originals kept in lib/data/Foundation-Mat-Swatches/); red-blue.jpg is a
// crop of the all-colors stack photo, since no Red/Blue close-up was
// provided. `hex` is pixel-averaged from each photo's first-named face and
// is only a fallback — every entry has a real photo.
export const FOUNDATION_MAT_COLORS = [
  { name: 'Black/Gray', hex: '#2f2e27', photo: '/color-catalog/foundation/black-gray.jpg' },
  { name: 'Red/Blue', hex: '#df4a50', photo: '/color-catalog/foundation/red-blue.jpg' },
  { name: 'Green/Gray', hex: '#7a9376', photo: '/color-catalog/foundation/green-gray.jpg' },
].map((d) => ({ ...d, brand: 'foundation', provenance: 'photo-verified' }));

const VERIFIED_PROVENANCE = new Set(['photo-verified', 'swatch-verified', 'bryan-provided']);
...
export function listBallPitBallColors() {
  return onlyVerified(BALL_PIT_BALL_COLORS);
}

export function listSlideColors() {
  return onlyVerified(SLIDE_COLORS);
}

export function listFoundationMatColors() {
  return onlyVerified(FOUNDATION_MAT_COLORS);
}
...
export function findBallPitBallByName(name) {
  return listBallPitBallColors().find((c) => c.name === name) || null;
}

export function findSlideByName(name) {
  return listSlideColors().find((c) => c.name === name) || null;
}

export function findFoundationMatByName(name) {
  return listFoundationMatColors().find((c) => c.name === name) || null;
}
...
export function resolveSelectedColor(selection) {
  ...
  if (selection.brand === 'plastic') return findSlideByName(selection.code);
  if (selection.brand === 'foundation') return findFoundationMatByName(selection.code);
  return null;
}
...
export function computeLineItemPricing(requiredInputs, selections) {
  ...
  for (const input of requiredInputs || []) {
    for (const part of input.parts) {
      ...
      } else if (selection?.brand === 'plastic') {
        // Flat per-color amount (only Gray, $500) — unlike Prismatic's
        // uniqueness-based first/additional rule above, every occurrence of
        // the same plastic color costs the same each time it's picked.
        amount = findSlideByName(selection.code)?.upcharge || 0;
      }
      lines.push({
