...
describe('computeLineItemPricing', () => {
  ...
  // Gray (+$500) was removed as a slide option on 2026-09-23.
  it('no longer offers or prices Gray (plastic)', () => {
    const slideInputs = [{ input: 'slide', label: 'Slide', parts: ['slide_platform', 'slide_color'] }];
    const lines = computeLineItemPricing(slideInputs, { slide: { slide_color: { brand: 'plastic', code: 'Gray' } } });
    expect(lines.map((l) => l.amount)).toEqual([0, 0]);
    expect(findSlideByName('Gray')).toBeNull();
  });
...
describe('Slide (plastic) catalog', () => {
  ...
  it('resolveSelectedColor handles the plastic brand', () => {
    const color = resolveSelectedColor({ brand: 'plastic', code: 'Green' });
