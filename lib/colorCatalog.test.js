...
import {
  ...
  findVinylByName,
  findSlideByName,
  listFoundationMatColors,
  findFoundationMatByName,
  cardinalFinishes,
  prismaticFinishes,
...
describe('color catalog — provenance gating', () => {
  ...
  it('loads the real, verified Prismatic dataset (378 colors — 428 minus 50 removed for having no derivable finish, 2026-09-03)', () => {
    expect(listPrismaticColors().length).toBe(378);
  });
...
// present in the stored 378 names/SKUs (not assumed): only those two are
// derivable — the other 6 real finish keywords (Solid Tone, Transparent,
// Metallic, Texture, Vein, Wrinkle) appear in zero stored names. The 50
// colors that had neither signal (a live spot-check found that dataset no
// longer matches the current site's SKU scheme at all) were removed from
// the catalog outright per Bryan's direct instruction (2026-09-03) — see
// lib/data/prismaticColors.json and lib/colorCatalog.js's header comment.
// derivePrismaticFinish still returns null defensively if a future catalog
// sync ever reintroduces an unclassifiable entry — finish stays null for
...
describe('Prismatic finish derivation (RAL / River, verified against the real site)', () => {
  ...
  it('leaves finish null (never guessed) if an entry has no RAL/River signal, but no such entries remain — the 50 that had none were removed outright, 2026-09-03', () => {
    expect(PRISMATIC_COLORS.find((c) => c.sku === 'PRB-10395')).toBeUndefined();
    for (const c of PRISMATIC_COLORS) {
...
describe('resolveSelectedColor', () => {
  ...
  it('resolves a real Prismatic selection back to its catalog entry', () => {
    const color = resolveSelectedColor({ brand: 'prismatic', code: 'PRB-2826' });
    expect(color?.name).toBe('Coppersun River');
  });
...
describe('computeLineItemPricing', () => {
  ...
  it('charges a repeated Prismatic color only once, even reused across parts', () => {
    const selections = {
      structure_frame_paint: {
        legs: { brand: 'prismatic', code: 'PRB-2826' },
        slide_color: { brand: 'prismatic', code: 'PRB-2826' },
      },
    };
    const lines = computeLineItemPricing(inputs, selections);
  ...
  // Slide (plastic) — real catalog and pricing given directly by Bryan
  // (2026-09-21): Blue/Green free, Gray +$500 flat, unlike Prismatic's
  // uniqueness-based first/additional rule above.
  it('prices Gray (plastic) at a flat $500, every time it is picked — not a first/additional rule', () => {
    const slideInputs = [{ input: 'slide', label: 'Slide', parts: ['slide_platform', 'slide_color'] }];
    const selections = {
      slide: {
        slide_platform: { brand: 'plastic', code: 'Gray' },
        slide_color: { brand: 'plastic', code: 'Gray' },
      },
    };
    const lines = computeLineItemPricing(slideInputs, selections);
    expect(lines.map((l) => l.amount)).toEqual([500, 500]);
  });

  it('prices Blue/Green (plastic) at $0', () => {
    const slideInputs = [{ input: 'slide', label: 'Slide', parts: ['slide_platform', 'slide_color'] }];
    const selections = {
      slide: {
...
describe('Slide (plastic) catalog', () => {
  it('has exactly Blue, Green, Gray', () => {
    const names = listSlideColors().map((c) => c.name).sort();
    expect(names).toEqual(['Blue', 'Gray', 'Green']);
  });

  it('findSlideByName resolves a real name and rejects a fake one', () => {
    expect(findSlideByName('Gray')?.upcharge).toBe(500);
    expect(findSlideByName('Blue')?.upcharge).toBe(0);
    expect(findSlideByName('Mauve')).toBeNull();
  });

  it('resolveSelectedColor handles the plastic brand', () => {
    const color = resolveSelectedColor({ brand: 'plastic', code: 'Gray' });
    expect(color?.name).toBe('Gray');
  });
});

describe('Foundation System mat catalog', () => {
  it('has exactly Black/Gray, Red/Blue, Green/Gray, each with a real photo', () => {
    const colors = listFoundationMatColors();
    expect(colors.map((c) => c.name)).toEqual(['Black/Gray', 'Red/Blue', 'Green/Gray']);
    colors.forEach((c) => expect(c.photo).toMatch(/^\/color-catalog\/foundation\//));
  });

  it('resolves only real names, only under the foundation brand', () => {
    expect(findFoundationMatByName('Red/Blue')?.brand).toBe('foundation');
    expect(findFoundationMatByName('Black')).toBeNull();
    expect(resolveSelectedColor({ brand: 'foundation', code: 'Green/Gray' })?.name).toBe('Green/Gray');
    expect(resolveSelectedColor({ brand: 'vinyl', code: 'Green/Gray' })).toBeNull();
  });

  it('carries no upcharge', () => {
    const inputs = [{ input: 'foundation_mat', label: 'Foundation System Mat', parts: ['foundation_mat'] }];
    const lines = computeLineItemPricing(inputs, { foundation_mat: { foundation_mat: { brand: 'foundation', code: 'Red/Blue' } } });
    expect(lines.map((l) => l.amount)).toEqual([0]);
  });
});
...
describe('displayColorName — Prismatic RAL/River disambiguation (170-color collision)', () => {
  ...
  it('leaves a non-duplicate Prismatic name exactly as-is, with no suffix appended', () => {
    const sample = PRISMATIC_COLORS.find((c) => c.sku === 'PRB-2826');
    expect(displayColorName(sample)).toBe('Coppersun River');
  });
