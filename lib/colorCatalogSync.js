...
// Deliberately http://, not https:// — checked directly (2026-09-07), not
// assumed: cardinalpaint.com's own https:// vhost 404s on this exact path
// (confirmed even with certificate validation disabled, so it's a real
// server-side routing difference, not a cert issue) while https://www.
// cardinalpaint.com/ itself resolves fine — only this specific path is
// affected. Switching to https:// would break this cron outright instead of
// only closing a low-severity, low-value MITM window against public paint
// color reference data. Re-check this if Cardinal ever fixes their TLS
// vhost config.
