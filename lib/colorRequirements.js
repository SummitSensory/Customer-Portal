 ...
 *
 * "Slide-Color" (plastic) and "Ball Pit-Ball Colors" / "Foundation
 * System-Mat Color" have real gate columns now but are NOT surfaced to
 * customers yet — see `buildable: false` below and
 * `unbuiltRequiredColorGates()`. There's no plastic catalog sourced for
 * Slide, no quantity-allocator UI for Ball Pit Balls, and no catalog data
 * at all for Foundation. A gate reading "Included" for one of these is a
 * real, unfulfilled requirement — surfaced to staff (admin panel), never
...
export const COLOR_INPUT = {
  ...
  SLIDE: 'slide', // plastic — not yet buildable, see header
};
 ...
 * entries for SLIDE/BALL_PIT_BALLS/FOUNDATION_MAT — see header; that means
 * any submission against those inputs is rejected outright (allowed = []),
...
const GATED_INPUTS = [
  ...
  },
  {
    ...
    buildable: false, // plastic — no catalog sourced yet
    legacyDefaultProductTypes: [ADVENTURE_SERIES],
  },
  {
    gateKey: 'soarMat',
 ...
 * requiredColorInputs) that have NO native picker built yet — Slide,
 * Ball Pit Balls, Foundation. Surfaced to staff (admin panel) so a real
 * requirement never silently goes unnoticed just because there's nowhere
 * for a customer to fulfill it yet.
 */
