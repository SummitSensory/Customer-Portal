...
import {
  ...
  unbuiltRequiredColorGates, COLOR_INPUT, ALLOWED_BRANDS,
...
describe('gate-driven requirements (Bryan\'s decision tree, 2026-09-21)', () => {
  it('with no colorGates/colorFrameType set (order predates these columns), every buildable gate AND the steel frame are required — fail closed, not fail open', () => {
    ...
    // Not-yet-buildable gates (Ball Pit Balls) never appear in
    // requiredInputs even when "Included" — see unbuiltRequiredColorGates.
    expect(keys).not.toContain(COLOR_INPUT.BALL_PIT_BALLS);
    // Foundation has no legacy default — unset means NOT required.
    expect(keys).not.toContain(COLOR_INPUT.FOUNDATION_MAT);
  });

  it('Foundation System Mat is buildable (real catalog sourced 2026-09-22) and only required when its gate is explicitly Included', () => {
    const included = requiredColorInputs({ productType: SOAR, colorGates: { foundationMat: 'Included' } });
    const foundation = included.find((i) => i.input === COLOR_INPUT.FOUNDATION_MAT);
    expect(foundation).toBeTruthy();
    expect(foundation.parts).toEqual(['foundation_mat']);
    expect(ALLOWED_BRANDS[COLOR_INPUT.FOUNDATION_MAT]).toEqual(['foundation']);

    const notIncluded = requiredColorInputs({ productType: SOAR, colorGates: { foundationMat: 'NOT Included' } });
    expect(notIncluded.find((i) => i.input === COLOR_INPUT.FOUNDATION_MAT)).toBeUndefined();
  });

  it('Slide: the platform is painted (Cardinal/Prismatic), only the slide itself is plastic — both under the Slide-Color gate', () => {
    const inputs = requiredColorInputs(allIncludedOrder(ADVENTURE_SERIES));
    const slide = inputs.find((i) => i.input === COLOR_INPUT.SLIDE);
    const platform = inputs.find((i) => i.input === COLOR_INPUT.SLIDE_PLATFORM);
    expect(slide.parts).toEqual(['slide_color']);
    expect(platform.parts).toEqual(['slide_platform']);
    expect(ALLOWED_BRANDS[COLOR_INPUT.SLIDE]).toEqual(['plastic']);
    expect(ALLOWED_BRANDS[COLOR_INPUT.SLIDE_PLATFORM]).toEqual(['cardinal', 'prismatic']);

    const off = requiredColorInputs({ productType: ADVENTURE_SERIES, colorGates: { slideColor: 'NOT Included' } });
    expect(off.find((i) => i.input === COLOR_INPUT.SLIDE)).toBeUndefined();
    expect(off.find((i) => i.input === COLOR_INPUT.SLIDE_PLATFORM)).toBeUndefined();
  });

  it('Ball Pit is one color', () => {
    const inputs = requiredColorInputs(allIncludedOrder(ADVENTURE_SERIES));
    expect(inputs.find((i) => i.input === COLOR_INPUT.BALL_PIT).parts).toEqual(['ball_pit_vinyl']);
  });

  it('90 Degree Climb & Slide: 3 vinyl mat pieces with the numbered image, only when Included', () => {
    expect(requiredColorInputs({ productType: ADVENTURE_SERIES }).find((i) => i.input === COLOR_INPUT.CLIMB_SLIDE)).toBeUndefined();
    const inputs = requiredColorInputs({ productType: SOAR, colorGates: { climbSlideMat: 'Included' } });
    const cs = inputs.find((i) => i.input === COLOR_INPUT.CLIMB_SLIDE);
    expect(cs.parts).toEqual(['climb_slide_piece_1', 'climb_slide_piece_2', 'climb_slide_piece_3']);
    expect(cs.image).toBe('/color-catalog/climb-slide/90-degree-climb-slide.png');
    expect(ALLOWED_BRANDS[COLOR_INPUT.CLIMB_SLIDE]).toEqual(['vinyl']);
  });
  ...
  it('Adventure Mat System is ONE vinyl color for floor padding, column wraps and zip line (Bryan, 2026-09-23)', () => {
    const inputs = requiredColorInputs(allIncludedOrder(ADVENTURE_SERIES));
    const adventureMat = inputs.find((i) => i.input === COLOR_INPUT.ADVENTURE_MAT);
    expect(adventureMat.parts).toEqual(['adventure_mat_system']);
    expect(ALLOWED_BRANDS[COLOR_INPUT.ADVENTURE_MAT]).toEqual(['vinyl']);
    const steel = inputs.find((i) => i.input === COLOR_INPUT.STRUCTURE_FRAME_PAINT);
    expect(steel.parts).not.toContain('adventure_mat_system');
  });
...
describe('unbuiltRequiredColorGates — real requirements with no picker built yet', () => {
  ...
  it('flags Ball Pit Balls as Included but unbuilt — never silently dropped, never shown to the customer', () => {
    const order = { productType: ADVENTURE_SERIES, colorGates: {
      ballPitBalls: 'Included', foundationMat: 'Included',
    } };
    const gaps = unbuiltRequiredColorGates(order);
    expect(gaps.map((g) => g.gateKey)).toEqual(['ballPitBalls']);

    const inputs = requiredColorInputs(order);
    expect(inputs.find((i) => i.input === COLOR_INPUT.BALL_PIT_BALLS)).toBeUndefined();
    // Foundation is buildable now (2026-09-22) — shown to the customer, not a staff-only gap.
    expect(inputs.find((i) => i.input === COLOR_INPUT.FOUNDATION_MAT)).toBeTruthy();
  });
...
describe('allKnownInputParts', () => {
  it('unions parts across every productType/gate/frame type that shares the same input key', () => {
    ...
    expect(merged[COLOR_INPUT.ADVENTURE_MAT]).toEqual(['adventure_mat_system', 'zip_line']);
