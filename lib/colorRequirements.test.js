...
describe('gate-driven requirements (Bryan\'s decision tree, 2026-09-21)', () => {
  it('with no colorGates set (order predates these columns), every buildable gate is required — fail closed, not fail open', () => {
    ...
    expect(keys).toEqual(expect.arrayContaining([
      ...
      COLOR_INPUT.WALL_PADDING, COLOR_INPUT.BALL_PIT,
    ]));
    // Not-yet-buildable gates (Slide, Ball Pit Balls, Foundation) never
    // appear in requiredInputs even when "Included" — see the buildable
    // test below and unbuiltRequiredColorGates.
    expect(keys).not.toContain(COLOR_INPUT.SLIDE);
...
describe('unbuiltRequiredColorGates — real requirements with no picker built yet', () => {
  it('is empty when nothing not-yet-buildable is Included', () => {
    const order = { productType: ADVENTURE_SERIES, colorGates: {
      slideColor: 'NOT Included', ballPitBalls: 'NOT Included', foundationMat: 'NOT Included',
    } };
    expect(unbuiltRequiredColorGates(order)).toEqual([]);
  });

  it('flags Slide/Ball Pit Balls/Foundation as Included but unbuilt — never silently dropped, never shown to the customer', () => {
    const order = { productType: ADVENTURE_SERIES, colorGates: {
      slideColor: 'Included', ballPitBalls: 'Included', foundationMat: 'Included',
    } };
    const gaps = unbuiltRequiredColorGates(order);
    expect(gaps.map((g) => g.gateKey)).toEqual(expect.arrayContaining(['slideColor', 'ballPitBalls', 'foundationMat']));

    const inputs = requiredColorInputs(order);
    expect(inputs.find((i) => i.input === COLOR_INPUT.SLIDE)).toBeUndefined();
