...
describe('color requirements manifest — exact-label matching', () => {
  ...
  it('recognizes the real Adventure Series label', () => {
    ...
    expect(inputs[0].parts).toEqual([
      'legs', 'horizontal_beams', 'ladder_rungs_and_leg', 'slide_platform', 'slide_color', 'climbing_wall', 'zip_line',
    ]);
  });

  it('Adventure Series also requires a Climbing Wall Mat vinyl color, associated with (but distinct from) the climbing wall frame paint', () => {
    // Direct requirement from Bryan (2026-09-09).
    const inputs = requiredColorInputs('Summit Adventure Series: Custom Sensory Gym');
    const mat = inputs.find((i) => i.input === COLOR_INPUT.CLIMBING_WALL_MAT);
    expect(mat).toBeTruthy();
    expect(mat.parts).toEqual(['climbing_wall_mat']);
  });

  it('Adventure Series also requires Column Wraps & Pads and Ball Pit as native vinyl inputs', () => {
    // Migrated 2026-09-09 — verified against the real Jotform color form
    // (243436740186156) via the Jotform API; previously Jotform-only and,
    // since the "color" tab switches entirely to the native picker per
    // order, unreachable by any Adventure Series customer since the native
    // picker went live. See lib/colorRequirements.js's own comment on this.
    const inputs = requiredColorInputs('Summit Adventure Series: Custom Sensory Gym');
    const wraps = inputs.find((i) => i.input === COLOR_INPUT.MAT_PAD_COLOR);
    expect(wraps).toBeTruthy();
    expect(wraps.parts).toEqual(['column_wraps_pads']);

    const ballPit = inputs.find((i) => i.input === COLOR_INPUT.BALL_PIT);
    expect(ballPit).toBeTruthy();
    expect(ballPit.parts).toEqual([
  ...
  it('both Flex labels also require a Floor Pad vinyl color', () => {
    // Migrated 2026-09-09 — verified against the real Jotform color form
    // (243426572564158); same orphaning issue as Adventure Series' Column
    // Wraps & Pads / Ball Pit, just a single part here.
    for (const label of ['Summit Flex: Universal Exercise Unit', 'Summit Flex: Universal Exercise Unit & Accessories']) {
      const inputs = requiredColorInputs(label);
      const floorPad = inputs.find((i) => i.input === COLOR_INPUT.MAT_PAD_COLOR);
      expect(floorPad).toBeTruthy();
      expect(floorPad.parts).toEqual(['floor_pad']);
    }
  });
  ...
  it('Soar also requires Column Wraps & Floor Padding as a native vinyl input', () => {
    // Migrated 2026-09-08 — previously Jotform-only and, since the "color"
    // tab switches entirely to the native picker per order, unreachable by
    // any Soar customer from 2026-09-03 onward. See lib/colorRequirements.js.
    const inputs = requiredColorInputs('Summit Soar: Mobile Free-Standing Swing Frame');
    const wraps = inputs.find((i) => i.input === COLOR_INPUT.MAT_PAD_COLOR);
    expect(wraps).toBeTruthy();
    expect(wraps.parts).toEqual(['column_wraps', 'floor_padding']);
  });
...
describe('allKnownInputParts', () => {
  ...
  it('includes mat_pad_color parts across both Therapy Mats & Pads and Soar', () => {
    const merged = allKnownInputParts();
    expect(merged[COLOR_INPUT.MAT_PAD_COLOR]).toEqual(
      expect.arrayContaining(['mat_pad', 'column_wraps', 'floor_padding'])
    );
  });
