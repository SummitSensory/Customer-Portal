...
import { findCardinalByCode, findPrismaticBySku, findVinylByName, findSlideByName, findFoundationMatByName, computeLineItemPricing } from './colorCatalog';
...
export function validatePartSelection(inputType, partKey, selection) {
  ...
  if (brand === 'foundation') {
    if (!code || !findFoundationMatByName(code)) return `"${partKey}" has an unrecognized Foundation System mat color.`;
    return null;
  }
