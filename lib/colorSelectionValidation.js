...
import { requiredColorInputs, ALLOWED_BRANDS } from './colorRequirements';
 ...
 * ONLY the exact input/part keys this order's productType actually has, and
 * ONLY the two whitelisted fields off each value — never a spread/copy of
 * the raw client-supplied `selections`. Found by independent code review
 * (2026-09-02): validatePresentSelections/validateColorSelectionData only
 * ever WALK the known keys to check them; neither one rejects extra keys or
 * oversized values sitting alongside them, and the raw object was persisted
 * to Monday's snapshot column as-is. This is the actual enforcement step —
 * anything not in the whitelist is silently dropped before it can ever
 * reach storage, regardless of what the request body contained.
 */
export function sanitizeSelections(order, selections) {
  ...
  for (const input of inputs) {
    for (const part of input.parts) {
      const value = selections?.[input.input]?.[part];
      if (value && typeof value === 'object' && typeof value.brand === 'string' && typeof value.code === 'string') {
        if (!clean[input.input]) clean[input.input] = {};
        clean[input.input][part] = { brand: value.brand, code: value.code };
      }
    }
  }
  return clean;
