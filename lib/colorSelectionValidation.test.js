...
// 2026-09-23 layout change: a customer's already-saved picks for retired parts
// (Diverse Options had zip_line, a plastic slide_platform and 2 Ball Pit
// sections) must survive the next autosave rather than be silently deleted.
describe('retired parts survive sanitizeSelections', () => {
  it('keeps zip_line, the old plastic slide_platform and Ball Pit sections', () => {
    const order = { productType: 'Summit Adventure Series: Custom Sensory Gym' };
    const clean = sanitizeSelections(order, {
      adventure_mat: { zip_line: { brand: 'vinyl', code: 'Lime' } },
      slide: { slide_platform: { brand: 'plastic', code: 'Blue' }, slide_color: { brand: 'plastic', code: 'Green' } },
      ball_pit: { ball_pit_vinyl: { brand: 'vinyl', code: 'Yellow' }, mat_section_1: { brand: 'vinyl', code: 'Lime' } },
    });
    expect(clean.adventure_mat.zip_line.code).toBe('Lime');
    expect(clean.slide.slide_platform.code).toBe('Blue');
    expect(clean.ball_pit.mat_section_1.code).toBe('Lime');
  });
});
