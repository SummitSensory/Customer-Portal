...
export async function notifyTeamNewReferral(orderName, referrerEmail, friendName, friendEmail) {
  await send({
    from: FROM,
    to: TEAM_EMAIL,
    ...
    html: shell(`
      ...
      ${btn('View Referrals Board in Monday', `${PORTAL_URL}/admin`)}
    `),
  });
}

export async function notifyTeamUgcThreshold(orderName, customerEmail, photoCount, videoCount, credits) {
  await send({
    from: FROM,
    to: TEAM_EMAIL,
    ...
    html: shell(`
      ...
      ${btn('Review Submissions in Monday', `${PORTAL_URL}/admin`)}
    `),
  });
}
