...
export async function notifyTeamNewReferral(orderName, referrerEmail, friendName, friendEmail, referralItemId) {
  ...
  await send({
    ...
    html: shell(`
      ...
      ${p(`<strong>${referrerEmail}</strong> (order <strong>${orderName}</strong>) referred a friend:`)}
      ${callout(`${friendName} — ${friendEmail}`, { icon: '🎁' })}
...
export async function notifyTeamUgcThreshold(orderName, customerEmail, photoCount, videoCount, credits, orderItemId) {
  ...
  await send({
    ...
    html: shell(`
      ...
      ${p(`<strong>${customerEmail}</strong> (order <strong>${orderName}</strong>) has submitted enough photos/videos to qualify for another $25 reward tier.`)}
