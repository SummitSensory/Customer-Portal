...
// PORTAL-034: several templates below interpolate customer-submitted free
// text (a changed contact-info field name, a referral's friend name/email,
// a portal chat message preview) directly into this HTML, unescaped. An
// authenticated customer's own session can reach every one of those fields
// (setup.js's delivery-tab `changedRestricted`, referral/submit.js,
// monday/messages.js), so a crafted value could inject a live, clickable
// link into a real, trusted-domain email landing in Summit's own inbox —
// this is a different rendering path than the Messages-tab stored-XSS fix
// (lib/sanitizeHtml.js), which only covers the in-app thread, not these
// transactional emails. Escape any such value before it reaches a template.
...
export async function sendPortalInvitation(email, customerName, orderName) {
  ...
  await send({
    ...
    html: shell(`
      ...
      ${p(`We're excited to get started! Your project with <strong>${orderName}</strong> is officially underway, and your secure Summit Sensory Gym customer portal is now ready.`)}
    ...
    `, { preheader: `Access your customer portal for ${orderName}` }),
  });
}
...
export async function notifyCustomerFreightUpdate(email, customerName, orderName, shipmentLabel, statusLabel, trackingUrl) {
  ...
  await send({
    ...
    html: shell(`
      ...
      ${h1(msg.headline)}
      ${p(`Hi ${name},`)}
      ${p(`Here's the latest on your <strong>${shipmentLabel}</strong> shipment for order <strong>${orderName}</strong>:`)}
      <div style="background:${isException ? '#FEF2F2' : C.mossLt};border-radius:12px;padding:16px 20px;margin:20px 0;text-align:center">
        <span style="font-family:Arial,sans-serif;font-size:13px;color:${C.muted};font-weight:600;text-transform:uppercase;letter-spacing:.06em">Current Status</span><br />
        <span style="font-family:Arial,sans-serif;font-size:20px;font-weight:800;color:${isException ? '#B91C1C' : C.moss}">${statusLabel}</span>
      </div>
      ...
      ${btn('Track Shipment', trackingUrl || `${PORTAL_URL}/portal`)}
      ${divider()}
    ...
    `, { preheader: `${shipmentLabel} — ${statusLabel} — ${orderName}` }),
  });
}
          ...
          <td style="padding:16px 20px">
            <table cellpadding="0" cellspacing="0" border="0">
              <tr>
                ...
                <td style="font-family:Arial,sans-serif;font-size:15px;font-weight:600;color:${C.ink};vertical-align:middle">${fileName}</td>
              </tr>
            </table>
          </td>
        </tr>
