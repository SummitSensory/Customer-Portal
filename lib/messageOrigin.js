 ...
 *
 * Anchored to the LEADING tag only — pages/api/monday/messages.js (read-only
 * reference; not edited here) always writes a chat message body as exactly
 * `[PORTAL]${originTag}\n${body}`, i.e. the fixed `[PORTAL]` prefix
 * immediately followed by `[PORTAL:STAFF]` or `[PORTAL:CUSTOMER]`, then the
 * customer/staff-typed text. The previous version used
 * body.includes('[PORTAL:STAFF]') / .includes('[PORTAL:CUSTOMER]') —
 * unanchored substring search over the ENTIRE body, including the
 * free-typed message text that follows the real tag. A customer whose own
 * message happened to contain the literal substring "[PORTAL:STAFF]"
 * anywhere in their text (e.g. quoting/describing this exact bug) would be
 * misclassified as staff-authored. Checking startsWith on the two full,
 * literal leading-tag strings makes this immune to anything appearing later
 * in the body, staff or customer.
 */
export function isStaffMessage(msg) {
  const body = msg?.body || '';
  if (body.startsWith('[PORTAL][PORTAL:STAFF]')) return true;
  if (body.startsWith('[PORTAL][PORTAL:CUSTOMER]')) return false;
