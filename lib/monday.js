...
export async function addFileToOrder(itemId, fileUrl, fileName) {
  ...
  if (!fileRes.ok) {
    throw new Error(`addFileToOrder: failed to download ${fileUrl} — ${fileRes.status} ${fileRes.statusText}`);
  }
  const buffer = Buffer.from(await fileRes.arrayBuffer());
  const mimeType = fileRes.headers.get('content-type') || 'application/octet-stream';
  const resolvedName = fileName || fileUrl.split('/').pop()?.split('?')[0] || 'file';
  return uploadFileToColumn(itemId, COLS.portalFiles, buffer, resolvedName, mimeType);
}
...
export async function attachUgcFile(itemId, fileUrl, type) {
  ...
  const fileRes = await fetch(fileUrl);
  if (!fileRes.ok) {
    throw new Error(`attachUgcFile: failed to download ${fileUrl} — ${fileRes.status} ${fileRes.statusText}`);
  }
  const buffer = Buffer.from(await fileRes.arrayBuffer());
  ...
  return uploadFileToColumn(itemId, colId, buffer, fileName, mimeType);
}
