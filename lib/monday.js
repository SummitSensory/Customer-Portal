...
function parseOrderItem(item) {
  ...
  return {
    ...
    // Color form: env var override first, then exhaustive mirror column extraction.
    // The mirror column may source a Link-type column, so the URL can be buried
    // in several different places inside the value JSON blob.
    colorFormId: (() => {
      // Env var hard-override (bypasses Monday entirely)
