...
// Labels written to the Tax Exempt? status column (color_mm55tjn2). If the
// labels on that column in Monday don't already read "Yes"/"No", either
// rename them to match or override via these env vars — create_labels_if_missing
...
export const COLS = {
  ...
  // Invite automation — Status column that triggers the auto-invite ("Send Invite" → "Invite Sent")
  inviteStatus:     process.env.MONDAY_COL_INVITE_STATUS  || 'color_mm5427cr',
  // Tax exemption — customer-facing Yes/No selector + certificate upload (Invoice & Payment tab)
  taxExemptStatus:   process.env.MONDAY_COL_TAX_EXEMPT_STATUS || 'color_mm55tjn2', // "Tax Exempt?" (status column)
  taxExemptCertFile: process.env.MONDAY_COL_TAX_EXEMPT_CERT   || 'file_mm55t6kn',  // "Tax Exemption Certificate" (file column)
...
/** Set any Status column to a given label by COLS key (creates the label if missing). */
...
function parseOrderItem(item) {
  ...
  return {
    ...
    // Tax exemption — "Yes" / "No" / "" (not yet answered)
    ...
    installationVideos: getText('installationVideos') || '',
