...
async function mondayQuery(query, variables = {}) {
  // Only retry reads, not mutations: a mutation that times out may have
  // already been applied server-side even though we never saw the
  // response — blindly retrying it risks a duplicate write (double-created
  // item, double column update, etc). Reads are safe to retry once.
...
export async function markSectionCompleteSafe(itemId, colKey) {
  ...
  try {
    ...
    console.error(`markSectionComplete retry 2/2 failed for item ${itemId}, column "${colKey}" — giving up, checklist sync is now out of date:`, err.message);
    return false;
  }
}
...
function isAllowedUploadHost(hostname) {
  const h = (hostname || '').toLowerCase();
  return UPLOAD_HOST_ALLOWLIST.some(allowed => h === allowed || h.endsWith(`.${allowed}`));
}

async function safeFetchUploadedFile(fileUrl, { context = 'file download' } = {}) {
  let parsed;
  try {
  ...
  if (!isAllowedUploadHost(parsed.hostname)) {
    throw new Error(`${context}: rejected URL from disallowed host "${parsed.hostname}" (allowlist: ${UPLOAD_HOST_ALLOWLIST.join(', ')}; override via UPLOAD_HOST_ALLOWLIST)`);
  }
...
export async function addFileToOrder(itemId, fileUrl, fileName) {
  const { buffer, mimeType } = await safeFetchUploadedFile(fileUrl, { context: 'addFileToOrder' });
