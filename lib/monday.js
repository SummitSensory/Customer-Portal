...
function parseOrderItem(item) {
  ...
  for (const col of item.column_values || []) {
    colMap[col.id] = { text: col.text, value: col.value, display_value: col.display_value };
  }
  ...
  const getMirror = (colKey) => {
    ...
    if (!col) return '';
    // display_value is set by the MirrorValue GraphQL fragment — most reliable for lookup columns
    if (col.display_value?.trim()) return col.display_value.trim();
    // Fallback: text field (works for some column types)
    if (col.text?.trim()) return col.text.trim();
    // Last resort: parse the raw JSON value blob
    try {
