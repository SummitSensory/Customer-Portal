...
export const COLS = {
  ...
  // Color & Product Selections snapshot (2026-09-01, native picker rollout).
  // Unlike deliverySnapshot/billingSnapshot above, this column does NOT
  // exist yet on the live Manufacturing Process board — it has been
  // deliberately left unmapped (no hardcoded fallback id) until it's
  // created. writeColorSelectionSnapshot()/parseOrderItem() below both
  // treat an unset value as "not yet configured" and fail loudly rather
  // than silently no-op, matching the class of bug already fixed once for
  // updateTrackingNumber()/updateBalance() (see
  // claude-project-docs/fresh-audit-and-fixes-2026-08-27.md #4).
  colorSelectionSnapshot: process.env.MONDAY_COL_COLOR_SNAPSHOT || null,
  ...
  // Color Selection — per-item requirement gating columns (2026-09-21,
  // Bryan's "Color Selection Decision Tree" spreadsheet — real column IDs
  // he supplied directly, not a guess). Each one is an INDEPENDENT
  // Included/NOT Included status column — confirmed explicitly by Bryan:
  // not derived from productType, not derived from colorFrameType, and not
  // derived from any of the other gate columns. "Included" / "NOT Included"
  // are the exact live values he confirmed. See lib/colorRequirements.js
  // for how these are consumed — unset/blank/unrecognized fails CLOSED
  // (treated as required), matching this file's established policy of
  // never silently skipping a real requirement.
  // IDs below are the MIRROR columns on Manufacturing Process (lookup_*) of
  // the real status columns, which live on Deal Tracking (6527740233) under
  // color_mm7c* IDs. Those color_mm7c* IDs don't exist on this board, so
  // reading them always returned blank and every gate silently fell back to
  // its legacy default (found 2026-09-23 against the live boards). Labels:
  // Included / NOT Included, with "Required" as the unset default.
  colorFrameType:       process.env.MONDAY_COL_COLOR_FRAME_TYPE      || 'lookup_mm7c11rs', // "Product Type" (Adventure/Soar/Flex) — informational only, does NOT gate any requirement
  colorAdventureMat:    process.env.MONDAY_COL_COLOR_ADVENTURE_MAT   || 'lookup_mm7cqhkf', // "Adventure-Mat Color" (vinyl; includes Zip Line per Bryan 2026-09-21)
  colorClimbingWall:    process.env.MONDAY_COL_COLOR_CLIMBING_WALL   || 'lookup_mm7czm49', // "Climbing Wall-Color" (paint — Cardinal/Prismatic)
  colorClimbingWallMat: process.env.MONDAY_COL_COLOR_CLIMBING_WALL_MAT || 'lookup_mm7ctj4j', // "Climbing Wall-Mat Color" (vinyl)
  colorWallPaddingMat:  process.env.MONDAY_COL_COLOR_WALL_PADDING_MAT || 'lookup_mm7cw3hn', // "Wall Padding-Mat Color" (vinyl, its own separate item per Bryan 2026-09-21)
  colorSlide:           process.env.MONDAY_COL_COLOR_SLIDE           || 'lookup_mm7c87q5', // "Slide-Color" (plastic — Blue/Green/Gray, sourced 2026-09-21)
  colorSoarMat:         process.env.MONDAY_COL_COLOR_SOAR_MAT        || 'lookup_mm7ct0z3', // "Soar-Mat Color" (vinyl)
  colorFlexMat:         process.env.MONDAY_COL_COLOR_FLEX_MAT        || 'lookup_mm7c9m7f', // "Flex-Mat Color" (vinyl)
  colorPalisadesMat:    process.env.MONDAY_COL_COLOR_PALISADES_MAT   || 'lookup_mm7caez6', // "Palisades-Mat Color" (vinyl)
  colorBallPitMat:      process.env.MONDAY_COL_COLOR_BALL_PIT_MAT    || 'lookup_mm7cp0yb', // "Ball Pit-Mat Color" (vinyl)
  colorBallPitBalls:    process.env.MONDAY_COL_COLOR_BALL_PIT_BALLS  || 'lookup_mm7cc6p0', // "Ball Pit-Ball Colors" (vinyl — no quantity-allocator UI yet; gate exists, not shown to customers)
  colorClimbSlideMat:   process.env.MONDAY_COL_COLOR_CLIMB_SLIDE_MAT || 'lookup_mm7gv7pq', // "90 Deg Climb/Slide-Mat Color" mirror of Deal Tracking color_mm7g1481 (created 2026-09-23)
  colorFoundationMat:   process.env.MONDAY_COL_COLOR_FOUNDATION_MAT  || 'lookup_mm7ccp4a', // "Foundation System-Mat Color" (two-tone foam tile — Black/Gray, Red/Blue, Green/Gray, sourced 2026-09-22)
};

// Mirror (lookup) columns return text: null and value: null on this API
// version — their content is ONLY in display_value, via a MirrorValue
// fragment. Requested for just the mirrors parseOrderItem reads (not all ~70
// on the board) because resolving a mirror is comparatively expensive, and
// getAllOrders pulls every order. Before this, every one of these silently
// read as blank: the color gates, the bill-to address on file, the Delivery
// POC name/phone/email, and the primary contact.
const MIRROR_COL_KEYS = [
  'phone', 'pocName', 'pocEmail', 'firstName', 'deliveryInstructions',
  'contactName', 'contactEmail', 'contactPhone',
  'billingAddressOnFile', 'billingZipOnFile',
  // Deliberately NOT fetched: trackingNumber/freightTracking (GB FedEx
  // mirrors — reading them would newly feed the Order Status tracker and
  // AfterShip for orders that don't use them today) and colorFormId (its
  // reader below only looks at text/value, so Jotform routing is unchanged).
  'colorFrameType', 'colorAdventureMat', 'colorClimbingWall', 'colorClimbingWallMat',
  'colorWallPaddingMat', 'colorSlide', 'colorSoarMat', 'colorFlexMat',
  'colorPalisadesMat', 'colorBallPitMat', 'colorBallPitBalls', 'colorFoundationMat', 'colorClimbSlideMat',
];
const MIRROR_COL_IDS = [...new Set(MIRROR_COL_KEYS.map((k) => COLS[k]).filter(Boolean))];
const ORDER_MIRROR_FIELDS = `mirror_values: column_values(ids: ${JSON.stringify(MIRROR_COL_IDS)}) { id ... on MirrorValue { display_value } }`;
...
//
// PORTAL-057: 15000 was too tight for the Manufacturing Process board's
// current size. Live Vercel logs showed getAllOrders() (a full items_page
// query with column_values + subitems for every order — the heaviest query
// this file makes) timing out on essentially every accessory-tracking-sync
// cron run for 13 straight days (first seen 2026-09-04), and by 2026-09-17
// the same AbortError had spread to single-item getOrderById lookups too
// (/api/monday/order, /api/monday/orders) — which is the exact call
// pages/api/aftership/track.js makes before it ever talks to AfterShip, so
// this surfaced to customers as "AfterShip tracking never updates" even
// though AfterShip itself was never the broken part. Raised to give Monday
// room to answer the heavier queries; paired with the retry-with-backoff
// below (mondayQuery's catch) since a timeout can also be a genuine
// transient hiccup that a bigger ceiling alone wouldn't fix.
const MONDAY_FETCH_TIMEOUT_MS = 30000;
...
export async function mondayQuery(query, variables = {}, attempt = 0) {
  ...
  try {
    ...
    // PORTAL-057: this used to retry exactly once, immediately, with no
    // backoff — fine for a one-off network blip, but useless against the
    // sustained timeouts real logs showed (the same query timing out on
    // essentially every hourly cron run for 13 days straight). Reusing the
    // same backoff already proven out for Monday's 429/complexity-budget
    // retries below gives a slow board a real chance to recover between
    // attempts instead of immediately re-hitting the same load.
    if (attempt < MONDAY_MAX_RETRIES) {
      const waitMs = MONDAY_RETRY_BASE_MS * (attempt + 1);
      console.warn(`Monday.com API request failed (${err.message}) — retrying in ${waitMs}ms (attempt ${attempt + 1}/${MONDAY_MAX_RETRIES})`);
      await new Promise(r => setTimeout(r, waitMs));
      return mondayQuery(query, variables, attempt + 1);
    }
    throw new Error(`Monday.com API request failed after ${MONDAY_MAX_RETRIES} retries: ${err.message}`);
  }
 ...
 * PORTAL-051: this used to be a single boards(limit: 50) call with no
 * pagination — unlike every board-*items* read in this file (all paged via
 * fetchAllBoardItems, per this file's own PORTAL-013-class fixes), a Monday
 * account with more than 50 accessible boards silently never showed boards
 * beyond the 50th in the admin picker. Monday's `boards` query paginates via
 * a 1-based `page` argument (not the items_page cursor style), so this pages
 * until a page returns fewer than `limit` results.
 */
export async function listBoards() {
  ...
  while (true) {
    ...
    if (data.boards.length < limit || page > 40) break; // same safety valve as fetchAllBoardItems
    page++;
  }
  return boards;
}
...
export async function getOrderByEmail(email, { fresh = false } = {}) {
  const orders = await getOrdersByEmail(email, { fresh });
 ...
 */
// PORTAL-060 follow-up: `fresh: true` bypasses the board cache entirely —
// added specifically for pages/api/auth/send-code.js's order-existence
// check, which gates whether a real login-code email gets sent at all. A
// stale cached "no order" result there silently skips a real send while
// still returning {sent:true} to the customer, for up to the cache's full
// TTL after a brand-new order is created. Every other caller (admin
// dashboard, crons, ordinary getOrdersByEmail lookups) is unaffected —
// they don't pass this option, so they keep the cached-read behavior this
// cache exists for.
// Deliberately WITHOUT ORDER_MIRROR_FIELDS: resolving ~20 mirrors for every
// order on the board (~380) pushed this query past the 30s fetch timeout
// (prod outage 2026-09-24 02:01–: logins, crons and the AfterShip webhook all
// failed). Mirrors are only fetched per order, in getOrderById — which is
// what every customer-facing portal page loads the order through.
export async function getAllOrders({ fresh = false } = {}) {
  if (fresh) {
...
// PORTAL-018-IDEMPOTENCY-FOLLOWUP: pages/api/portal/setup.js's in-memory
// recentDeliverySubmissions guard (see its own header comment) only closes
// the duplicate-row risk within a single warm serverless instance — a retry
// that lands on a different cold-started instance, or after this instance
// has recycled, sails right past it. This is the "fully robust version"
// that comment already called out as still-needed: it actually queries the
// Delivery & Site Details Submissions board itself for a recent matching
// row before a new one gets created, so the guarantee holds across
// instances/cold-starts too. The in-memory guard stays as-is (cheap,
// catches the common case with zero extra Monday API calls); this is the
// slower, authoritative backstop layered on top for the same-content-within-
// the-window case specifically.
const DELIVERY_SUBMISSION_DEDUPE_WINDOW_MS = 60_000;

// Fingerprints the subset of submitted fields that round-trip cleanly
// through Monday's column `text` representation (plain text/long_text
// columns and the Yes/No-label columns) — deliberately excludes fields
// backed by richer column types (the two "Can Text" boolean/checkbox
// columns) where reconstructing an exact match from `.text` alone is
// unreliable. Matching on this subset is intentionally close to exhaustive
// rather than a loose few-field check: this backstop's whole job is
// avoiding a FALSE POSITIVE (silently dropping a genuinely different
// submission), so it should only ever collapse a true retry of identical
// content, never two different real submissions that happen to share a POC
// name or address.
function deliverySubmissionFingerprint(fields) {
  return JSON.stringify([
    fields.pocName || '', fields.pocPhone || '', fields.pocEmail || '',
    fields.specialInstructions || '',
    fields.hasSecondaryPoc ? 'Yes' : 'No',
    fields.secondaryPocName || '', fields.secondaryPocPhone || '', fields.secondaryPocEmail || '',
    Array.isArray(fields.primaryCommMethods) ? fields.primaryCommMethods.join(', ') : (fields.primaryCommMethods || ''),
    Array.isArray(fields.secondaryCommMethods) ? fields.secondaryCommMethods.join(', ') : (fields.secondaryCommMethods || ''),
    fields.addressConfirmed === true ? 'Yes' : fields.addressConfirmed === false ? 'No' : '',
    fields.addressLine1 || '', fields.addressLine2 || '', fields.addressCity || '',
    fields.addressState || '', fields.addressZip || '', fields.addressCountry || '',
    fields.formattedAddress || '',
    fields.loadingDock || '', fields.deliveryTiming || '',
    fields.freightAckBy || '',
    Array.isArray(fields.changedRestricted) ? fields.changedRestricted.join(', ') : (fields.changedRestricted || ''),
  ]);
}

/**
 * The Monday-side (cross-instance, cross-cold-start) half of the delivery
 * submission idempotency guard — see the comment above
 * DELIVERY_SUBMISSION_DEDUPE_WINDOW_MS. Looks for an existing item for the
 * same order, created within the dedupe window, whose stored field values
 * fingerprint-match the incoming submission exactly. Returns the matching
 * item, or null if this is not a duplicate (including whenever
 * DELIVERY_BOARD_ID isn't configured — same fail-open-to-"not a duplicate"
 * stance findRecentReferral takes, since treating every submission as a
 * false duplicate on a misconfigured board would silently drop real ones).
 */
export async function findRecentDeliverySubmission(orderId, fields) {
  if (!DELIVERY_BOARD_ID || !orderId) return null;
  const items = await fetchAllBoardItems(DELIVERY_BOARD_ID, `
    id name created_at
    column_values { id text value }
  `);
  const wantedOrderId = String(orderId);
  const wantedFingerprint = deliverySubmissionFingerprint(fields);
  const cutoff = Date.now() - DELIVERY_SUBMISSION_DEDUPE_WINDOW_MS;
  ...
  for (const item of items) {
    ...
    const cmap = {};
    for (const col of item.column_values || []) cmap[col.id] = { text: col.text, value: col.value };
    const itemOrderId = (cmap[DELIVERY_COLS.orderItemId]?.text || '').trim();
    if (itemOrderId !== wantedOrderId) continue;

    const itemAddrConfirmed = cmap[DELIVERY_COLS.addressConfirmed]?.text;
    const itemFingerprint = deliverySubmissionFingerprint({
      pocName: cmap[DELIVERY_COLS.primaryPocName]?.text || '',
      pocPhone: cmap[DELIVERY_COLS.primaryPocPhone]?.text || '',
      pocEmail: cmap[DELIVERY_COLS.primaryPocEmail]?.text || '',
      specialInstructions: cmap[DELIVERY_COLS.specialInstructions]?.text || '',
      hasSecondaryPoc: cmap[DELIVERY_COLS.hasSecondaryPoc]?.text === 'Yes',
      secondaryPocName: cmap[DELIVERY_COLS.secondaryPocName]?.text || '',
      secondaryPocPhone: cmap[DELIVERY_COLS.secondaryPocPhone]?.text || '',
      secondaryPocEmail: cmap[DELIVERY_COLS.secondaryPocEmail]?.text || '',
      primaryCommMethods: cmap[DELIVERY_COLS.primaryCommMethods]?.text || '',
      secondaryCommMethods: cmap[DELIVERY_COLS.secondaryCommMethods]?.text || '',
      addressConfirmed: itemAddrConfirmed === 'Yes' ? true : itemAddrConfirmed === 'No' ? false : undefined,
      addressLine1: cmap[DELIVERY_COLS.addressLine1]?.text || '',
      addressLine2: cmap[DELIVERY_COLS.addressLine2]?.text || '',
      addressCity: cmap[DELIVERY_COLS.city]?.text || '',
      addressState: cmap[DELIVERY_COLS.stateProvince]?.text || '',
      addressZip: cmap[DELIVERY_COLS.postalCode]?.text || '',
      addressCountry: cmap[DELIVERY_COLS.country]?.text || '',
      formattedAddress: cmap[DELIVERY_COLS.formattedAddress]?.text || '',
      loadingDock: cmap[DELIVERY_COLS.loadingDock]?.text || '',
      deliveryTiming: cmap[DELIVERY_COLS.deliveryTiming]?.text || '',
      freightAckBy: cmap[DELIVERY_COLS.freightAckBy]?.text || '',
      changedRestricted: cmap[DELIVERY_COLS.restrictedChanges]?.text || '',
    });

    if (itemFingerprint === wantedFingerprint) return { id: item.id, name: item.name };
  }
  return null;
}
...
export async function createDeliverySubmissionItem(order, fields) {
  ...
  const writes = [
    ...
    // Summit's business date, not UTC — a 6pm-Mountain submission used to
    // land on the board dated the next day.
    [DELIVERY_COLS.submittedDate, { date: new Date().toLocaleDateString('en-CA', { timeZone: 'America/Denver' }) }],
  ...
  // Deliberately create the item BLANK and fill it afterwards (rather than
  // one create_item with column_values): the board has "when an item is
  // created" Monday AI automations that extract City/State/Zip from Full
  // Ship-To Address Formatted. Creating the item already populated would
  // hand those automations real input and let them overwrite the
  // customer's exact City/State/Zip with an AI re-parse.
  ...
  // PORTAL-062: this loop used to be a bare `.catch(err => console.error(...))`
  // per write with nothing tracking which ones actually failed — unlike the
  // near-identical loop in createReferralItem() above (PORTAL-016), a
  // partial Monday outage mid-loop here left a delivery/site-details
  // submission with some fields silently blank (secondary POC, comm
  // preferences, loading dock, the ship-to address itself) and nothing on
  // the board distinguished that from a customer who genuinely left those
  // fields empty. Applies the exact same failedCols-tracking + visible
  // tagged-update pattern createReferralItem() already uses, so staff
  // reviewing a submission can tell "really blank" from "failed to save."
  const failedCols = [];
  for (const [colId, value] of writes) {
    await updateItemColumnOnBoard(DELIVERY_BOARD_ID, itemId, colId, value)
      .catch(err => {
        console.error(`Delivery submission column write failed (${colId}):`, err.message);
        failedCols.push(colId);
      });
  }
  if (failedCols.length > 0) {
    await postTaggedUpdate(
      itemId,
      'PORTAL: Delivery Submission Incomplete',
      `${failedCols.length} field(s) failed to save during this delivery/site-details submission and may be missing/blank below: ${failedCols.join(', ')}. Please verify with the customer before relying on this submission (e.g. for the ship-to address or freight ack).`
    ).catch(err => console.error('Failed to post delivery-submission-incomplete flag update:', err.message));
  }
  return itemId;
}
...
/**
 * Persist a customer's color/finish selections. Follows the same
 * not-best-effort rule PLAN-12 established for deliverySnapshot/
 * billingSnapshot: a failure here must fail the whole request, never be
 * swallowed into a false "saved" response.
 *
 * Deliberately does NOT fall back to a hardcoded column id the way
 * deliverySnapshot/billingSnapshot do — that column does not exist on the
 * live board yet (see COLS.colorSelectionSnapshot above). Throws a clear,
 * specific error if it's not configured, instead of writing to an
 * env-var-typo'd or nonexistent column and getting an opaque Monday error.
 */
export async function writeColorSelectionSnapshot(itemId, data) {
  if (!COLS.colorSelectionSnapshot) {
    throw new Error(
      'Color selection storage is not yet configured (MONDAY_COL_COLOR_SNAPSHOT unset) — ' +
      'the Manufacturing Process board needs a new long_text column for this, created and mapped before this endpoint can save anything.'
    );
  }
  await updateOrderColumn(itemId, COLS.colorSelectionSnapshot, { text: JSON.stringify(data) });
}
...
/**
 * Find the accessory subitem matching a carrier slug + tracking number.
 * Used by the AfterShip webhook, which only knows slug + tracking number,
 * not which subitem/order they belong to.
 *
 * PORTAL-038: this used to run its own uncached fetchAllBoardItems() call on
 * every single invocation, bypassing the exact board-read cache
 * (withBoardCache, in getAllAccessoryItems()) that exists specifically
 * because concurrent full-board reads caused redundant Monday calls — see
 * that function's own PORTAL-025 comment tying it directly to the
 * documented AfterShip 429 storm. A burst of AfterShip checkpoint webhooks
 * (the exact historical trigger for that storm) re-opened the same
 * uncached-read pattern here. Scanning getAllAccessoryItems()'s cached,
 * already-parsed results instead means a webhook burst shares the same
 * 20s-cached board read as everything else, not a fresh one per event.
 */
export async function findAccessorySubitemByTracking(slug, trackingNumber) {
  ...
  const match = items.find((item) =>
    item.trackingNumber === wantedNumber && (!wantedSlug || item.carrierSlug.toLowerCase() === wantedSlug)
  );
 ...
 * PORTAL-038: this used to run its own uncached fetchAllBoardItems() call on
 * every single invocation, bypassing the exact board-read cache
 * (withBoardCache, in getAllOrders()) that exists specifically because
 * concurrent full-board reads caused redundant Monday calls — see that
 * function's own PORTAL-025 comment tying it directly to the documented
 * AfterShip 429 storm. A burst of AfterShip checkpoint webhooks (the exact
 * historical trigger for that storm) re-opened the same uncached-read
 * pattern here. Scanning getAllOrders()'s cached, already-parsed results
 * instead means a webhook burst shares the same board-read cache as
 * everything else. frameCarrierSlug/frameTrackingId/matsCarrierSlug/
 * matsTrackingId/customerEmail/contactName/freightNotifyEnabled are already
 * parsed top-level fields (see parseOrderItem); only the "last notified"
 * tag columns aren't, so those still come from the order's own rawColumns.
 */
export async function findOrderByFreightTracking(slug, trackingNumber) {
  ...
  for (const order of orders) {
    const frameNumber = (order.frameTrackingId || '').trim();
    const frameSlug = (order.frameCarrierSlug || '').trim().toLowerCase();
    const matsNumber = (order.matsTrackingId || '').trim();
    const matsSlug = (order.matsCarrierSlug || '').trim().toLowerCase();
    ...
    return {
      ...
      freightNotifyEnabled: !!order.freightNotifyEnabled,
      lastNotifiedTag: order.rawColumns?.[notifyTagCol]?.text?.trim() || '',
    };
  }
  return null;
}
...
export async function updateFreightNotifyTag(itemId, shipmentKey, tag) {
  ...
  // PORTAL-060: findOrderByFreightTracking() (the AfterShip webhook's dedupe
  // check) reads order.lastNotifiedTag straight out of getAllOrders()'s
  // 20-second board cache (boardCache/withBoardCache, above) — that cache is
  // never told this write just changed the exact column it's reading a
  // stale copy of. AfterShip fires a webhook per checkpoint, not per unique
  // status change, so a burst of repeated checkpoint webhooks for the same
  // shipment landing within that same 20s window all read the OLD tag and
  // each re-send the customer email. Invalidating the 'orders' cache entry
  // the moment this write actually lands means the very next read is
  // correct instead of only "eventually" correct — reuses the same
  // boardCache.delete() mechanism withBoardCache's own failed-fetch path
  // (above) already uses, rather than inventing a second cache-invalidation
  // scheme.
  boardCache.delete('orders');
...
/**
 * Resolves the real, customer-approved delivery contact(s) for an order —
 * who AfterShip should actually be told to notify about a Frame/Mats
 * shipment (see lib/aftership.js's `contacts` param). Direct requirement
 * from Bryan (2026-09-03): AfterShip needs the email the customer
 * specifically approved as their delivery point of contact, not just
 * whoever's logged into the portal.
 *
 * Prefers the customer's own most-recent Delivery tab submission
 * (order.deliverySnapshot — see pages/api/portal/setup.js's 'delivery' case)
 * over the read-only CRM mirror columns (pocName/pocEmail/phone, sourced
 * from the connected Deal board) — same "the customer's own confirmed value
 * wins over the stale mirror" precedent already used for
 * billingAddressOnFile above, since a customer may correct their delivery
 * contact after the CRM record was first created and the mirror won't
 * reflect that.
 *
 * Returns { primary: {name, email, phone}|null, secondary: {...}|null }.
 * secondary is only populated when the customer explicitly turned on a
 * secondary contact on their most recent Delivery tab submission
 * (hasSecondaryPoc) — there is no secondary-contact mirror to fall back to.
 *
 * Gated on order.freightNotifyEnabled ("Freight Email Alerts", the Order
 * Status tab checkbox) — fixed 2026-09-03 after an independent review
 * flagged a real consent gap: this function's only two callers
 * (pages/api/aftership/track.js, pages/api/cron/accessory-tracking-sync.js)
 * feed AfterShip's own native notification flows, which have no concept of
 * that checkbox and will email anyone registered as a customer on a
 * tracking regardless of it. The only real lever available is to never
 * hand AfterShip a real email/phone for someone who opted out in the first
 * place. Gated HERE rather than at each call site so a future third caller
 * can't forget it and silently start emailing someone who said no. A
 * customer who opts out AFTER already being registered stays registered
 * until the next onboarding call for that shipment (the next portal visit
 * or the hourly cron) overwrites it with a bare, contact-free entry — a
 * small, disclosed timing gap, not an unbounded one.
 */
export function resolveDeliveryContacts(order) {
  if (!order?.freightNotifyEnabled) return { primary: null, secondary: null };

  const snap = order?.deliverySnapshot || null;

  const primary = {
    name: snap?.pocName || order?.pocName || '',
    email: snap?.pocEmail || order?.pocEmail || '',
    phone: snap?.pocPhone || order?.phone || '',
  };
  const hasPrimary = Boolean(primary.name || primary.email || primary.phone);

  let secondary = null;
  if (snap?.hasSecondaryPoc && (snap.secondaryPocName || snap.secondaryPocEmail || snap.secondaryPocPhone)) {
    secondary = {
      name: snap.secondaryPocName || '',
      email: snap.secondaryPocEmail || '',
      phone: snap.secondaryPocPhone || '',
    };
  }

  return { primary: hasPrimary ? primary : null, secondary };
}
...
export async function getAllAccessoryItems() {
  return withBoardCache('accessoryItems', BOARD_CACHE_TTL_MS, async () => {
    // parent_item added 2026-09-03: this board is Monday's own native
    // subitems board for Manufacturing Process ("Subitems of Manufacturing
    // Process", confirmed via the live board schema — NOT a loosely-linked
    // separate board), so every item here has a guaranteed, reliable
    // parent_item — unlike the GB/Manufacturing board_relation links
    // CLAUDE.md flags as unreliable elsewhere. Needed so AfterShip can be
    // told the real customer delivery contact for an accessory shipment,
    // not just the bare item name (see resolveDeliveryContacts + this
    // function's caller in pages/api/cron/accessory-tracking-sync.js).
...
// Dedup for customer notifications that can now be triggered from more than
// one place observing the same underlying Monday write — e.g. a status/
// balance change made through the Admin Portal (pages/api/monday/orders.js)
// and the direct-Monday-edit webhook (status-balance-webhook.js) both react
// to the exact same column write, so without this a staff edit made through
// the Admin Portal would email the customer once from that PATCH handler and
// again a few seconds later once Monday's own automation calls the webhook.
// Reuses the same tagged-update bookkeeping every other "don't repeat this"
// check in this file already relies on (cron/reminders.js's reminder count,
// AfterShip's lastNotifiedTag) instead of a new Monday column, so this needs
// no board schema change.
export async function hasNotifiedValue(itemId, kind, value) {
  const tag = `[PORTAL: ${kind} Notified - ${value}]`;
  const updates = await getOrderMessages(itemId);
  return updates.some(u => (u.body || '').startsWith(tag));
}

export async function markNotifiedValue(itemId, kind, value) {
  await postTaggedUpdate(
    itemId,
    `PORTAL: ${kind} Notified - ${value}`,
    `${kind} change to "${value}" was emailed to the customer on ${new Date().toLocaleDateString()}.`
  );
}

/**
 * PORTAL-059: hasNotifiedValue()/markNotifiedValue() above are a plain
 * check-then-act pair with no atomicity — pages/api/monday/orders.js's PATCH
 * handler and pages/api/monday/status-balance-webhook.js both react to the
 * exact same underlying Monday column write (a staff edit made through the
 * Admin Portal flips the same Manufacturing Phase/balance column the
 * automation-driven webhook watches), and both call hasNotifiedValue() then,
 * separately, markNotifiedValue(). Two requests that each read "not
 * notified" before either one's markNotifiedValue() write lands both proceed
 * to send — a real duplicate customer email, not a hypothetical one.
 *
 * Follows the same re-read-immediately-before-write plus post-write
 * verification shape pages/api/portal/color-selection.js already uses for
 * its own confirm-race (see that file's 2026-09-02/09-03 comments): a single
 * check at the top of a request can't be trusted by the time the request
 * gets around to acting on it, so check again as close to the actual send/
 * write as this architecture allows, and treat what lands after the write as
 * the source of truth for whether a race actually happened.
 *
 * Monday's API has no compare-and-swap on a text/update column (the same
 * hard platform constraint color-selection.js's own comments document), so
 * this narrows the race window to the gap between the re-check and the
 * write rather than eliminating it outright. What CAN be done: after
 * sending and marking, re-scan for how many "Notified - value" markers now
 * exist. Exactly one is the expected, healthy outcome. More than one means a
 * second concurrent caller's re-check also ran before this call's mark
 * landed — a real duplicate email almost certainly went out — so staff are
 * alerted via reportCriticalFailure() instead of that going fully silent.
 *
 * Both call sites (orders.js, status-balance-webhook.js) now go through this
 * single function instead of each doing their own
 * hasNotifiedValue/send/markNotifiedValue sequence, so the fix only needs to
 * live in one place. `sendFn` is responsible for its own error handling if a
 * failed send should still be marked notified (matches orders.js's existing
 * "attempt the email, but always record the marker" behavior) — if `sendFn`
 * throws, this function does NOT call markNotifiedValue, matching
 * status-balance-webhook.js's existing behavior of leaving the marker unset
 * on a failed send so a later retry can still notify.
 */
export async function sendCustomerNotificationOnce(itemId, kind, value, sendFn) {
  if (await hasNotifiedValue(itemId, kind, value)) {
    return { sent: false, alreadyNotified: true };
  }

  // Narrow the window: re-check immediately before acting, mirroring
  // color-selection.js's re-read-immediately-before-write pattern.
  if (await hasNotifiedValue(itemId, kind, value)) {
    return { sent: false, alreadyNotified: true };
  }

  await sendFn();
  await markNotifiedValue(itemId, kind, value);

  try {
    const tag = `[PORTAL: ${kind} Notified - ${value}]`;
    const updates = await getOrderMessages(itemId);
    const markerCount = updates.filter(u => (u.body || '').startsWith(tag)).length;
    if (markerCount > 1) {
      await reportCriticalFailure(
        'sendCustomerNotificationOnce-race',
        `Order ${itemId}: ${markerCount} "${kind} Notified - ${value}" markers found — a concurrent request likely raced past the dedupe check and the customer may have been emailed about this ${kind.toLowerCase()} change more than once.`,
        { itemId, kind, value, markerCount }
      );
    }
  } catch (err) {
    // A failed verification READ doesn't mean the send/mark above failed —
    // they already succeeded. Log and move on rather than turning a
    // successful notification into an error.
    console.error('sendCustomerNotificationOnce: post-write race verification failed:', err.message);
  }
...
function validateUploadUrl(urlStr, allowlist, context) {
  let parsed;
  try {
    parsed = new URL(urlStr);
  } catch {
    throw new Error(`${context}: invalid URL "${urlStr}"`);
  }
  if (parsed.protocol !== 'https:') {
    throw new Error(`${context}: rejected non-https URL "${urlStr}"`);
  }
...
// A hard cap on redirect hops this server will follow while resolving an
// upload URL — same defense-in-depth spirit as MAX_DOWNLOAD_BYTES below:
// bounds the cost of a malicious/misbehaving host looping redirects forever.
const MAX_UPLOAD_REDIRECT_HOPS = 5;

async function safeFetchUploadedFile(fileUrl, { context = 'file download', allowlist = UPLOAD_HOST_ALLOWLIST } = {}) {
  let parsed = validateUploadUrl(fileUrl, allowlist, context);

  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), DOWNLOAD_TIMEOUT_MS);
  let fileRes;
  try {
    // Real gap: fetch() follows redirects by default WITHOUT re-checking the
    // allowlist against the final destination — an allowlisted host (Jotform's
    // own domain, or the wider staff allowlist) that ever exposed an open
    // redirect could bounce this server-side download to an arbitrary/internal
    // address, defeating the allowlist check above entirely (the exact SSRF
    // class this function exists to prevent — see this section's own header).
    // Following redirects manually and re-validating each hop's Location
    // against the identical allowlist/https rules closes that gap.
    for (let hop = 0; ; hop++) {
      if (hop >= MAX_UPLOAD_REDIRECT_HOPS) {
        throw new Error(`${context}: too many redirects (>${MAX_UPLOAD_REDIRECT_HOPS}) resolving ${fileUrl}`);
      }
      let hopRes;
      try {
        hopRes = await fetch(parsed.toString(), { signal: controller.signal, redirect: 'manual' });
      } catch (err) {
      ...
      if (hopRes.status >= 300 && hopRes.status < 400 && hopRes.headers.get('location')) {
        ...
        try {
          ...
          throw new Error(`${context}: redirect to invalid URL "${location}" while resolving ${fileUrl}`);
        }
        parsed = validateUploadUrl(nextUrl.toString(), allowlist, context);
        continue;
      }
      fileRes = hopRes;
      break;
 ...
 *
 * PORTAL-049: item_id/column_id used to be inlined directly into the query
 * string ("this matches Monday's own documented example... both values
 * originate from our own COLS map / order lookups, never raw user input, so
 * there's no injection risk" — that reasoning held for columnId (always a
 * COLS/UGC_COLS constant) but NOT for itemId: pages/api/monday/files.js's
 * staff-only POST takes `orderId` straight from the request body with only a
 * truthiness check (no numeric/format validation) and passes it straight
 * through addFileToOrder -> here, landing unquoted in the mutation string —
 * the one place in this file that deviated from the "always use GraphQL
 * variables" pattern every other mutation follows. Both ids now travel as
 * real variables like everywhere else; the multipart spec only requires the
 * FILE itself to be referenced via `map`, not item_id/column_id.
 *
 * PORTAL-050: this also had no timeout and no 429/complexity-budget retry,
 * unlike every other outbound Monday call in this file (mondayQuery /
 * mondayFetchOnce, see PORTAL-021/025 above) — it talks to a different
 * endpoint (api.monday.com/v2/file, multipart) so it couldn't just call
 * mondayQuery directly, but the same protection applies here now.
 */
export async function uploadFileToColumn(itemId, columnId, fileBuffer, fileName, mimeType, attempt = 0) {
  ...
  form.append('variables', JSON.stringify({ itemId: String(itemId), columnId }));
  ...
  let res;
  try {
    res = await fetch('https://api.monday.com/v2/file', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${process.env.MONDAY_API_TOKEN}`,
      },
  ...
  if (res.status === 429 && attempt < MONDAY_MAX_RETRIES) {
    const waitMs = MONDAY_RETRY_BASE_MS * (attempt + 1);
    console.warn(`Monday.com file upload rate-limited (429) — retrying in ${waitMs}ms (attempt ${attempt + 1}/${MONDAY_MAX_RETRIES})`);
    await new Promise(r => setTimeout(r, waitMs));
    return uploadFileToColumn(itemId, columnId, fileBuffer, fileName, mimeType, attempt + 1);
  }
  ...
  if (json.errors) {
    if (isMondayThrottleError(json) && attempt < MONDAY_MAX_RETRIES) {
      const waitMs = MONDAY_RETRY_BASE_MS * (attempt + 1);
      console.warn(`Monday.com file upload complexity/rate limit hit — retrying in ${waitMs}ms (attempt ${attempt + 1}/${MONDAY_MAX_RETRIES})`);
      await new Promise(r => setTimeout(r, waitMs));
      return uploadFileToColumn(itemId, columnId, fileBuffer, fileName, mimeType, attempt + 1);
    }
    throw new Error(`Monday.com GraphQL error: ${json.errors[0].message}`);
 ...
 */
async function readUgcCounts(itemId, colIds) {
  const data = await mondayQuery(`
  ...
  return {
    photoCount: Number(colMap[UGC_COLS.photoCount]?.text) || 0,
    videoCount: Number(colMap[UGC_COLS.videoCount]?.text) || 0,
  };
}

/**
 * PORTAL-061: this is a classic read-modify-write with no lock and no
 * Monday-side compare-and-swap — the Jotform Showcase form can deliver
 * several webhook events for the same order close together (a customer
 * submitting multiple photos/videos in one multi-file form each arrive as
 * their own webhook call), and two concurrent calls that both read the same
 * "before" counts both compute their new totals off that same stale base.
 * Whichever write lands last silently wins and overwrites the other's
 * increment — a real counted photo/video quietly lost, with no signal to
 * staff that it happened.
 *
 * Re-reads the counters immediately before writing (same
 * re-read-immediately-before-write shape used elsewhere in this file/
 * pages/api/portal/color-selection.js) so the write is always based on the
 * freshest values this function can get, narrowing the window a second
 * concurrent call could land in. Monday has no compare-and-swap on a Number
 * column, so this can't be made fully atomic — if the two reads disagree,
 * that's proof a concurrent write landed in between (this call's own
 * increment may now be computed off values a THIRD write could still race
 * past), so it's reported via reportCriticalFailure() instead of silently
...
export async function incrementUgcCounts(itemId, photoDelta = 0, videoDelta = 0) {
  ...
  const initialRead = await readUgcCounts(itemId, colIds);
  // Narrow the window: re-read immediately before computing/writing, instead
  // of trusting a read taken at the top of this call that may now be stale.
  const freshRead = await readUgcCounts(itemId, colIds);
  const readsDisagree = freshRead.photoCount !== initialRead.photoCount || freshRead.videoCount !== initialRead.videoCount;

  const prevPhotoCount = freshRead.photoCount;
  const prevVideoCount = freshRead.videoCount;
  ...
  if (readsDisagree) {
    await reportCriticalFailure(
      ...
      `Order ${itemId}: UGC photo/video counters changed between two reads taken moments apart (photo ${initialRead.photoCount}->${freshRead.photoCount}, video ${initialRead.videoCount}->${freshRead.videoCount}) — a concurrent Showcase webhook delivery landed in between. This write used the freshest read available, but the counts may still be off if a third concurrent write landed after that. Worth a manual check of the real photo/video count on this order.`,
      { itemId, initialRead, freshRead, photoDelta, videoDelta }
    );
  }
...
/**
 * Get every update (message/audit-trail tag) on an order.
 * PORTAL-032: this used to hardcode `updates(limit: 50)` with no further
 * pagination — real chat plus every automated tag this app posts
 * (delivery/billing resubmissions, color confirmations, referrals,
 * reminders, invite logs, etc.) share this same update stream, so a
 * long-lived/active order can pass 50 updates easily. Every consumer of
 * this function needs the COMPLETE history, not just the newest 50:
 * Jotform's redelivery dedupe and multi-form tab-completion check
 * (pages/api/jotform/webhook.js) both scan for a specific marker anywhere
 * in the history, cron/reminders.js finds the invite marker to know when
 * the reminder clock started, and the Messages tab has no "load more" UI to
 * fall back on. Paginated via Monday's `page` param on the updates
 * connection (not the cursor-based items_page/next_items_page scheme
 * fetchAllBoardItems uses — that's specific to board items, not an item's
 * updates) until a page comes back short of a full page. MAX_PAGES is a
 * defensive cap (2,000 updates) against a runaway loop if Monday's
 * pagination ever misbehaves, matching fetchAllBoardItems's own
 * cursor-abort safety valve.
 */
export async function getOrderMessages(itemId) {
  ...
  for (let page = 1; page <= MAX_PAGES; page++) {
    ...
    `, { itemId: [itemId], limit: PAGE_SIZE, page });

    const pageUpdates = data.items?.[0]?.updates || [];
    allUpdates = allUpdates.concat(pageUpdates);
    if (pageUpdates.length < PAGE_SIZE) break;
    if (page === MAX_PAGES) {
      console.error(`getOrderMessages: aborting after ${MAX_PAGES} pages on item ${itemId} — unexpectedly large update history.`);
    }
  }
  return allUpdates;
}
...
async function parseOrderItem(item) {
  ...
  // See ORDER_MIRROR_FIELDS — mirrors carry their content only here.
  for (const col of item.mirror_values || []) {
    if (!colMap[col.id]) colMap[col.id] = {};
    colMap[col.id].display = col.display_value;
  }

  const get = (colKey) => colMap[COLS[colKey]]?.text || colMap[COLS[colKey]]?.display || '';

  // An order linked to several deals gets one value per deal, comma-joined:
  // "Christeen, Christeen", "80110, 80110", or a whole address repeated
  // ("4301 S Federal…, CO 80110, 4301 S Federal…, CO 80110"). collapseRepeats
  // undoes exact repetition without splitting a real comma-containing value;
  // firstOf additionally takes the first value for single-token fields
  // (names, emails, phones, zips), where a comma never belongs.
  const collapseRepeats = (v) => {
    const parts = String(v || '').split(', ');
    for (let k = 1; k < parts.length; k++) {
      if (parts.length % k) continue;
      const chunk = parts.slice(0, k).join(', ');
      let same = true;
      for (let i = k; i < parts.length; i += k) {
        if (parts.slice(i, i + k).join(', ') !== chunk) { same = false; break; }
      }
      if (same) return chunk;
    }
    return String(v || '');
  };
  const firstOf = (v) => String(v || '').split(',').map((x) => x.trim()).filter(Boolean)[0] || '';

  // A gate mirror joins one value per linked deal ("Included, Required"
  // when an order is linked to two deals). Any "Included" wins — never
  // silently drop a requirement; otherwise one agreed-on value is used, and
  // anything mixed/unknown is passed through for lib/colorRequirements.js
  // to treat as unset (its legacy-default fallback).
  const getGate = (colKey) => {
    const parts = [...new Set(get(colKey).split(',').map((v) => v.trim()).filter(Boolean))];
    if (parts.includes('Included')) return 'Included';
  ...
  const getMirror = (colKey) => {
    ...
    if (!col) return '';
    if (col.text?.trim()) return col.text.trim();
    if (col.display?.trim()) return collapseRepeats(col.display.trim());
  ...
  return {
    ...
    // Color Selection gating (2026-09-21) — raw status text per gate
    // column, read the same way productType is above. The fail-closed
    // "blank/unset = required" interpretation lives in
    // lib/colorRequirements.js, not here — this is just the raw read.
    colorFrameType: getGate('colorFrameType'),
    colorGates: {
      adventureMat: getGate('colorAdventureMat'),
      climbingWallColor: getGate('colorClimbingWall'),
      climbingWallMat: getGate('colorClimbingWallMat'),
      wallPaddingMat: getGate('colorWallPaddingMat'),
      slideColor: getGate('colorSlide'),
      soarMat: getGate('colorSoarMat'),
      flexMat: getGate('colorFlexMat'),
      palisadesMat: getGate('colorPalisadesMat'),
      ballPitMat: getGate('colorBallPitMat'),
      ballPitBalls: getGate('colorBallPitBalls'),
      foundationMat: getGate('colorFoundationMat'),
      climbSlideMat: getGate('colorClimbSlideMat'),
    },
    // Mirror columns — use getMirror() to handle Monday.com's JSON value format
    phone: firstOf(getMirror('phone')),
    pocName: firstOf(getMirror('pocName')),
    pocEmail: firstOf(getMirror('pocEmail')),
    firstName: firstOf(getMirror('firstName')),
    ...
    contactName: firstOf(getMirror('contactName')),
    contactEmail: firstOf(getMirror('contactEmail')),
    contactPhone: firstOf(getMirror('contactPhone')),
    ...
    colorSelectionSnapshot: (() => {
      try { const raw = getText('colorSelectionSnapshot'); return raw ? JSON.parse(raw) : null; }
      catch { return null; }
    })(),
    // FIXED (found in independent code review, 2026-09-01): before this,
    // pages/portal/index.js routed to the native color picker based only on
    // productType — with zero awareness that COLS.colorSelectionSnapshot
    // could be unconfigured, which made writeColorSelectionSnapshot() throw
    // on every single save. Verified blast radius: the picker loaded and
    // let a customer browse/select colors normally, but every save
    // (autosave or confirm) failed with no fallback to the still-working
    // Jotform — a full, silent regression for every order of a supported
    // product type on any deployment where that column hadn't been created
    // yet. This flag is a pure env check (no extra Monday API call, and the
    // order object is already being fetched before tabs render), so the
    // portal can fall back to the Jotform-based ColorTab automatically
    // instead of routing customers into a picker that can't actually save.
    colorSelectionWritable: !!COLS.colorSelectionSnapshot,
...
function parseAccessorySubitem(subitem) {
  ...
  return {
    ...
    // Only present when this was fetched via getAllAccessoryItems() (which
    // requests parent_item) — null when parsed as an order's own nested
    // subitems (parseOrderItem already knows its own id in that context, so
    // it never needs this field).
    orderId: subitem.parent_item?.id || null,
  };
}
