...
const MIRROR_COL_KEYS = [
  ...
  // trackingNumber/freightTracking (GB FedEx mirrors) deliberately left out:
  // reading them would newly feed the Order Status tracker and AfterShip for
  // orders that don't use them today — a separate change to make on purpose.
  'colorFormId',
...
async function parseOrderItem(item) {
  ...
  const getMirror = (colKey) => {
    ...
    if (!col) return '';
    if (col.text?.trim()) return col.text.trim();
    if (col.display?.trim()) return col.display.trim();
