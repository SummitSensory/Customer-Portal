...
function parseOrderItem(item) {
  ...
  const getMirror = (colKey) => {
    ...
    try {
      ...
      // Format 3: { linkedPulseIds: [{ linkedPulseId: ..., columnValues: [{ text }] }] }
      if (Array.isArray(parsed.linkedPulseIds)) {
        for (const lp of parsed.linkedPulseIds) {
          if (Array.isArray(lp?.columnValues)) {
            for (const cv of lp.columnValues) {
  ...
  return {
    ...
    colorFormId: getMirror('colorFormId'),
